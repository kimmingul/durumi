...
### 7.0 커버리지 게이트 — M1~M4에서 확인된 두 사실 (SPEC-2~5 공용)

**(a) vitest의 glob 임계값은 예외가 아니라 추가 그룹이다.** `vitest@2.1.9/dist/coverage.js`의 `resolveThresholds`는 **모든** 파일을 전역 임계값 맵에 무조건 추가한 뒤, glob 그룹을 그 위에 얹는다. 따라서 glob은 "이 파일만 다른 기준" 이 아니라 "이 파일에 기준을 하나 더" 다. 결과적으로 **어떤 파일을 게이트에서 빼려면 리포트에서도 빠진다** — 게이트 완화와 가시성 상실이 분리되지 않는다.

manager-develop이 첫 접근이 실패한 뒤 배포된 소스를 직접 읽어 확인했고, 불가능한 임계값을 넣어보는 탐침으로 재확인했다. 문서가 아니라 소스에서 확인해야 알 수 있는 동작이므로 기록한다.

**(b) 총계 임계값은 신규 미테스트 모듈을 잡지 못한다.** 게이트 대상 표면이 96.07%인 상태에서 테스트 없는 200줄 파일이 새로 들어오면 총계는 94.8%로 내려앉을 뿐 임계선을 넘지 않아 통과한다. **`perFile: true`가 게이트를 실제로 작동하게 만드는 설정**이다.

이는 vitest 특유의 문제가 아니라 커버리지 게이트 일반의 성질이다 — 총계는 이미 잘 덮인 표면 뒤에 새 구멍을 숨긴다. SPEC-2~5가 자기 표면에 게이트를 걸 때 같은 함정을 반복하지 않도록 여기 남긴다.
