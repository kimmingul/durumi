...
- **모순**: AC-WS-020은 조합 종료 후 버퍼가 "디스크 내용 + 조합 텍스트 모두 반영"을 요구했으나, 조합으로 커밋된 텍스트는 미저장 편집이므로 REQ-WS-028(사용자 확인 없이 미저장 편집 폐기 금지, 타협 불가)이 버퍼 교체를 금지한다. "둘 다 반영"은 §D가 제외한 3-way 병합이었다
- **결정 (사용자, 옵션 a)**: **REQ-WS-028 우선**. 조합 종료 후 자동 적용하지 않고 배너로 라우팅. AC-WS-020의 이전 문구는 **D-2 확정 이전에 작성된 잔재**이지 경쟁하는 의도가 아니었음을 기록
- **REQ-WS-021 재작성**: "조합 종료 = 적용" → **"조합 종료 = 정책 라우터로 라우팅 재개"**. 게이트 불변식 신설 — *게이트는 라우팅을 지연시킬 뿐 결과를 결정하지 않는다*
- **REQ-WS-023 보강**: 보류 표시의 종료 경로 명시 — 정책 결과(배너 또는 조용한 반영)로 **인계**되며 무성 소실 경로는 금지
- **AC 재작성/신설**: AC-WS-020(배너 라우팅), AC-WS-021(합류 대상은 라우팅이지 적용 아님) 재작성 + AC-WS-020b(텍스트 없는 조합 취소 → 자동 반영, 게이트 불변식 대조) / AC-WS-023c(보류 표시 인계, 무성 소실 FAIL) 신설
- **design.md §4.1 다이어그램 교체**: `compositionend ─► 적용` → 두 경로가 정책 라우터로 합류하는 형태
- **추가 발견 (지시 목록 외)**: design.md §7 메타데이터 다이어그램이 판 0.2.2의 값 기반 억제 전환을 반영하지 않고 "선언되어 있으면 억제"로 남아 있었음 — 함께 정정
- **REQ-WS-028 / §D 불변**: 어느 것도 약화하지 않았다. 옵션 (b)(조합 텍스트를 미저장 편집에서 제외)는 방금 고친 버그를 재생산하므로 채택하지 않음
- **요구사항 56개** (변동 없음), **수용 기준 83개 항목** (81 → 83)
...
| 파일 | 상태 | 내용 |
|---|---|---|
| `electron/externalWatch.ts` | 신규 | ConfirmedFileEvent → 렌더러 형태 변환 + 엄격 UTF-8 디코드 |
| `electron/ipc/project.ts` | 신규 | 5개 채널, 전부 assertAllowedPath 선행 |
| `src/store/externalChangeChannel.ts` | 신규 | 삭제·디코드실패·정상변경 3갈래 라우팅 |
| `src/hooks/useExternalChangeWiring.ts` | 신규 | 감시 등록·해제, dirty 전달, 구독 해제 |
| `shared/ipc-contract.ts` · `preload.ts` · `menu.ts` · `App.tsx` | 수정 | 채널 선언·브리지·메뉴 진입점·배선 |

| AC | ↔ REQ | 상태 | 근거 |
|---|---|---|---|
| AC-WS-058b | 047a | PASS | IPC 계약·핸들러·preload·MenuCommand·라우터·메뉴 6곳 확인 |
| AC-WS-019 | 020 | **PASS (CI)** | 조합 중 외부 내용 미반영. 표면 계측 3단 + 채널 생존 확인 선행 |
| AC-WS-022 | 022 | **PASS (CI)** | 조합 경계 유지 + 커밋 텍스트 존재 |
| AC-WS-020 | 021 | **FAIL (CI)** | SPEC 모순 — 아래 |
| AC-WS-021 | 021 | **FAIL (CI)** | 동일 |

**M8이 찾아낸 결함 3건**

1. **경계 누수** — 확정 계층이 추적하지 않던 경로를 "새 파일"로 확정하므로,
   `assertAllowed`에 실패한 경로의 이벤트도 읽혀 내용이 렌더러로 올라갔다.
   소유 목록으로 끊었다.
2. **파서 메시지 정보 누출** — js-yaml 오류 메시지가 문제된 소스 줄을 그대로
   붙인다. 손상된 매니페스트 내용이 `message`를 타고 경계를 넘었다.
   위협 모델 테스트가 잡았고, 첫 줄만 남기도록 정리했다.
3. **REQ-WS-028 위반 (가장 심각)** — `appStore.isDirty`가 조정 스토어로
   **한 번도 전달되지 않았다**(grep 0건). 정책이 언제나 깨끗한 버퍼로 판단해
   자동 반영하므로 미저장 편집이 확인 없이 사라진다. CI의 AC-WS-019 실패를
   3단계 계측(표면 도달 → 조합 유지 → held-composition 진입)으로 좁혀 찾았다.
   게이트는 정상이었고 원인은 배선 누락이었다.

**AC-WS-020 / 021이 드러낸 SPEC 모순 (미해결 — 판단 필요)**

REQ-WS-028 수정 후 두 AC가 timeout으로 실패한다. 실패가 아니라 **요구 충돌**이다:

- AC-WS-020: 조합 종료 후 보류된 조정이 적용되고 최종 버퍼가
  "디스크 내용 + 사용자가 커밋한 조합 텍스트"를 **모두** 반영한다.
- REQ-WS-027/028: 미저장 편집이 있으면 버퍼를 교체하지 **않고** 배너로 알린다.

조합으로 입력한 글자는 미저장 편집이다. 따라서 조합 종료 시점의 버퍼는 dirty이고,
REQ-WS-028을 지키면 조정은 배너로 가야 하며 디스크 내용이 버퍼에 들어올 수 없다.
AC-WS-020이 요구하는 "둘 다 반영"은 3-way 병합인데 spec.md §D가 명시적으로
범위 밖으로 두었다.

세 선택지 중 하나가 필요하다 — **구현자가 임의로 정하지 않고 올린다**:
 (a) AC-WS-020/021을 "조합 종료 후 배너가 뜬다"로 재작성 (REQ-WS-028 우선)
 (b) 조합 텍스트를 미저장 편집으로 세지 않는 예외 신설 (REQ-WS-028 약화)
 (c) 3-way 병합을 범위로 편입 (§D 변경)

**남은 것**: 위 결정과 그에 따른 AC 재작성. 그 전까지 AC-WS-020/021은 FAIL로 둔다 —
