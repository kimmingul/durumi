...
- 총 **83개 항목**. 식별자는 AC-WS-001 ~ 070이며, 013 / 016 / 020 / 023 / 031 / 038 / 039 / 057 / 058 / 067 열 개가 분할되어(013·013b, 016a·016b, **020·020b**, **023a·023b·023c**, 031·031b, 038·038b·038c, 039·039b, 057·057b, 058·058b, 067·067b·067c) 항목 수가 최대 번호보다 13 많다. **AC-WS-023은 단독 식별자로 존재하지 않는다** — 023a/023b/023c로 완전 대체되었다(마일스톤 밀폐성 + 보류 표시 인계).
...
### AC-WS-020 `[P+N]` 조합 종료 시 보류된 변경이 배너로 라우팅된다 ↔ REQ-WS-021, REQ-WS-027, REQ-WS-028
- **Given** AC-WS-019의 상태에서 — 문서가 열려 있고, `startComposition`으로 조합이 열린 채 유지되며, 그 사이 외부 변경이 확정되어 보류된 상태
- **When** `endComposition(handle)`이 호출되어 조합 텍스트가 커밋된다
- **Then** 버퍼는 **사용자가 커밋한 조합 텍스트를 그대로 담고 있다**
- **And** 디스크 내용이 버퍼에 적용되지 **않았다**
- **And** 조정 상태가 배너(notify) 상태이며, "차이 보기"와 "디스크에서 불러오기" 두 동작이 모두 제공된다
- **왜 "둘 다 반영"이 아닌가**: 커밋된 조합 텍스트는 미저장 편집이므로 조합 종료 시점의 버퍼는 dirty다. REQ-WS-028(사용자 확인 없이 미저장 편집 폐기 금지, 타협 불가)이 버퍼 교체를 금지하고, 두 내용을 합치는 것은 §D가 제외한 3-way 병합이다. 이 AC의 이전 판("디스크 내용 + 조합 텍스트 모두 반영")은 **D-2 확정 이전에 작성된 잔재**이며 경쟁하는 의도가 아니었다 — D-2가 "미저장 편집이 있으면 버퍼를 교체하지 않고 배너로 알린다"를 확정한 시점에 이미 무효였다

### AC-WS-021 `[P+N]` 보류 중 다중 변경은 최종 상태 1건으로 합류되어 라우팅된다 ↔ REQ-WS-021
- **Given** 조합이 열린 채 유지되는 동안 외부에서 같은 파일이 3회 바뀌어 3건이 확정된 상태
- **When** 조합이 종료된다
- **Then** 정책 라우터가 **1회** 호출되고, 그 호출이 참조하는 디스크 상태는 3회 중 **최종** 상태다
