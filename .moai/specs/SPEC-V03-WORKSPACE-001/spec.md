...
| 2026-08-07 | 0.2.3 | **M1~M4 구현이 드러낸 결함 7건 반영**. ① REQ-WS-014를 **2단계 확정**(전 경로 크기+수정시각 → 열린 파일만 내용 대조)으로 재작성해 AC-WS-013과의 모순 해소. ② REQ-WS-004의 "동일한 상한"을 상한 수치 한정으로 좁히고 정지 조건 차이 명시. ③ REQ-WS-055에 자체 대가 기록(`PROSPERO CRD` 리터럴 표현 불가). ④ REQ-WS-056 신설(읽을 수 없는 `bibliography` → 표시 동반 폴백). ⑤ REQ-WS-030에 사라짐 표시 해제 불가 명시 |
...
**When** 사용자가 파일을 열면, 앱은 해당 파일의 디렉터리에서 시작해 상위로 올라가며 매니페스트를 탐색**해야 한다(shall)**. 탐색 상한은 32단계**여야 하며(shall)**, 이 **상한 수치는** 기존 `.bib` walk-up 탐색(`electron/bibliography.ts`)과 동일하다.

**정지 조건은 동일하지 않다 (의도된 차이)**: `.bib` 탐색은 32단계 상한 **외에** 워크스페이스 루트에서도 멈춘다(`pickStopRoot`). 프로젝트 discovery는 그 조건을 채택하지 **않는다(shall not)** — 워크스페이스가 등록되지 않은 상태에서 파일을 열면 자기 디렉터리만 탐색하게 되어 REQ-WS-001/AC-WS-001이 깨지고, 프로젝트 없음 상태가 1급 상태라는 C-1과 충돌한다. 즉 두 탐색은 **상한만 공유하고 정지 조건은 다르다**.
...
**REQ-WS-014** (Ubiquitous) — 확정은 2단계다
앱은 감시 이벤트를 그대로 신뢰하지 **않고(shall not)**, 아래 2단계 재검사를 거쳐 변경을 확정**해야 한다(shall)**.

| 단계 | 대상 | 검사 | 비용 |
|---|---|---|---|
| **1단계 — 메타데이터 재검사** | 감시 중인 **모든** 경로 | 파일 크기와 수정 시각을 기록된 기준값과 대조. 일치하면 확정하지 않고 종료 | 파일을 읽지 않음 |
| **2단계 — 내용 대조** | **열려 있는 파일만**, 1단계를 통과한 경우에만 | 디스크 내용을 그 버퍼가 마지막으로 로드/저장한 내용과 대조. 동일하면 확정하지 **않는다(shall not)** | 추가 비용 없음 (아래 참조) |

**열린 파일의 외부 변경 확정은 두 단계를 모두 통과할 때만 성립한다(shall)**. 열려 있지 않은 감시 경로는 1단계만으로 확정되며, 그 확정은 폴더 수준 변경 신호(REQ-WS-045)이지 조정 대상이 아니다.

**2단계가 추가 비용을 만들지 않는 이유**: 열린 파일의 조정은 최소 차이 적용(REQ-WS-025)과 차이 보기(REQ-WS-027)를 위해 **어차피 디스크 내용을 읽어야 한다**. 2단계는 그 읽기를 확정 판정에 재사용할 뿐 새로운 읽기를 도입하지 않는다. 또한 대상이 **열린 파일로 한정**되므로, 그 파일은 이미 편집기 버퍼에 전량 적재되어 있다 — 즉 2단계의 비용 상한은 파일을 여는 시점에 이미 지불된 비용과 같은 차수이며, REQ-WS-046이 `data/` 폴더 감시를 배제한 이유(수 기가바이트 파일을 **읽지 않기 위해**)와 충돌하지 않는다. `data/` 안의 파일이라도 열려 있다면(REQ-WS-046 단서) 이미 메모리에 있다.

**1단계만으로는 부족한 이유**: 내용이 동일한 재작성(같은 내용으로 덮어쓰기, 일부 도구의 저장 동작)은 수정 시각을 바꾸므로 1단계를 통과한다. 1단계만으로 확정하면 내용이 바뀌지 않았는데도 조정이 수행되어 캐럿과 실행 취소 이력이 불필요하게 흔들린다.

**잔여 동작 (기록된 한계)**: 열려 있지 **않은** 감시 경로에 대해 내용 동일 재작성이 일어나면 1단계만 적용되므로 폴더 변경 신호가 산출된다. 이는 트리 재열거를 유발할 뿐 버퍼에 영향이 없고 재열거는 멱등이므로 수용한다 — 이 경우까지 없애려면 열려 있지 않은 파일을 읽어야 하고, 그것이 정확히 REQ-WS-046이 배제한 비용이다.
...
이 표시는 **해제할 수 없어야 한다(shall)** — REQ-WS-027의 외부 변경 배너와 달리 이것은 알림이 아니라 **디스크의 현재 사실을 반영하는 상태**다. 해제 가능하게 만들면 파일이 존재하지 않는데도 존재하는 것처럼 보이게 되어 사용자가 저장 가능 여부를 오판한다. 표시는 파일이 다시 나타나거나 문서가 닫힐 때 사라**진다(shall)**.

두 표면의 구분: **배너**(REQ-WS-027)는 "이런 일이 있었다"는 알림이므로 해제 가능하고, **상태 표시**(REQ-WS-030)는 "지금 이렇다"는 사실이므로 해제 불가능하다. 둘 다 모달이 아니다(REQ-WS-049).
...
**수용된 대가 (REQ-WS-042의 대가와 같은 형태)**: 이 판정 아래에서는 사용자가 **문자열 `PROSPERO CRD` 자체를 의도한 값으로** 쓸 수 없다 — 레지스트리는 정해졌으나 번호가 아직 배정되지 않은 상태를 그 문자열로 표현하려 해도 미기입으로 판정되어 매니페스트 기본값이 대신 채택된다. `ClinicalTrials.gov NCT`도 같다. 이는 REQ-WS-042가 기록한 "의도적으로 저자 없음을 표현할 수 없다"와 동일한 계열의 비용이며, 인지된 상태로 수용한다. 번호 미배정 상태를 표현해야 하면 `PROSPERO CRD (pending)` 처럼 placeholder와 구별되는 값을 쓴다.
...
**REQ-WS-056** (Event-driven — 실패 모드 감지)
**When** 매니페스트의 `bibliography`가 선언되었으나 그 경로를 읽을 수 없으면(파일 부재, 권한 거부, 디렉터리를 가리킴), 앱은 기존 walk-up 탐색으로 폴백**해야 하며(shall)**, **동시에 그 폴백 사실과 읽을 수 없었던 선언 경로를 사용자에게 표시해야 한다(shall)**. 앱은 조용히 폴백하지 **않는다(shall not)**.

두 요구를 함께 두는 이유: 폴백만 하면 매니페스트의 오타가 **다른 서지 파일로 조용히 해결되어** 사용자가 잘못된 참고문헌을 쓰고도 알아채지 못한다. 반대로 폴백 없이 실패시키면 손상된 매니페스트 하나가 서지 기능 전체를 정지시켜 REQ-WS-007(손상된 매니페스트에서도 편집·저장은 계속 가능)의 자세와 어긋난다. 표시를 동반한 폴백이 두 실패 모드를 모두 피한다 — 이는 REQ-WS-038이 형식 위반 값에 대해 취하는 자세(값을 버리지 않고 보존하며 경고)와 같은 형태다.
