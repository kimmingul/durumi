...
**실질적 귀결 (반드시 인지할 것)**: 조합으로 입력해 커밋된 텍스트는 그 자체가 미저장 편집이다. 따라서 **텍스트를 산출한 조합이 끝난 시점의 버퍼는 항상 dirty이며, 결과는 항상 배너다** — 디스크 내용이 자동으로 버퍼에 들어오는 경로는 없다. 조합이 취소되어 텍스트를 남기지 않았고 버퍼가 그 외에 clean이라면 REQ-WS-024의 자동 반영이 적용된다.
