...
| 2026-08-08 | 0.2.5 | **AC-WS-020b를 앱의 실제 dirty 의미론에 맞게 재작성 (사용자 결정: 옵션 b)**. `appStore.isDirty`가 sticky(`\|\| s.isDirty`)여서 조합 진입만으로 플래그가 latch되고 취소로 복구되지 않으므로, "조합 경유 clean 버퍼"는 도달 불가 — 게이트 불변식의 e2e 대비는 재구성 불가함을 명시하고 유닛 계층 방어를 기록. AC-WS-020b는 "취소된 조합도 보류 변경을 흘리지 않는다"(hold-and-forget 방지)로 재작성. AC-WS-019 전제 문구 정정, AC-WS-023c clean 분기 주석. sticky 플래그는 **issue #12**(CRLF 무결성은 **issue #11**)로 등록 — 둘 다 이 SPEC이 드러낸 기존 결함이며 v0.3 범위 밖 |
| 2026-08-08 | 0.2.4 | **M8이 드러낸 REQ-WS-028 ↔ AC-WS-020 모순 해소 (사용자 결정: REQ-WS-028 우선)**. REQ-WS-021을 "조합 종료 = 적용"에서 **"조합 종료 = 정책 라우터로 라우팅 재개"** 로 재작성 — 게이트는 라우팅을 지연시킬 뿐 결과를 결정하지 않는다. 조합으로 커밋된 텍스트는 미저장 편집이므로 결과는 항상 배너이며, "디스크 내용 + 조합 텍스트 모두 반영"은 §D가 제외한 3-way 병합이었다. AC-WS-020/021 재작성 + AC-WS-020b(조합 취소 → 자동 반영) / AC-WS-023c(보류 표시 인계) 신설. REQ-WS-023에 보류 표시 종료 경로 명시. AC-WS-020의 이전 문구는 D-2 확정 이전의 잔재로 기록 |
...
**REQ-WS-021** (Event-driven) — 조합 종료는 적용이 아니라 **라우팅 재개**다
**When** 조합이 종료되면(`compositionend`), 앱은 보류된 변경을 조정 정책 라우터(REQ-WS-029)로 **넘겨야 한다(shall)**. 앱은 그 변경을 정책을 우회해 버퍼에 직접 적용하지 **않는다(shall not)**.

보류 중 같은 파일에 복수의 변경이 확정된 경우, 최종 디스크 상태 하나만 라우터로 넘긴다.

**게이트 불변식**: IME 게이트는 라우팅을 **지연**시킬 뿐 **결과를 결정하지 않는다(shall not)**. 조합 종료 후의 결과는 조합이 없었을 때와 동일한 규칙 — 미저장 편집이 없으면 반영(REQ-WS-024), 있으면 배너(REQ-WS-027) — 으로 결정**된다(shall)**.

**실질적 귀결 (반드시 인지할 것)**: 조합으로 입력해 커밋된 텍스트는 그 자체가 미저장 편집이다. 따라서 **텍스트를 산출한 조합이 끝난 시점의 버퍼는 항상 dirty이며, 결과는 항상 배너다** — 디스크 내용이 자동으로 버퍼에 들어오는 경로는 없다.

원리적으로는 조합이 취소되어 텍스트를 남기지 않았고 버퍼가 그 외에 clean이면 REQ-WS-024의 자동 반영이 적용된다. **다만 이 분기는 현재 앱에서 도달 불가능하다** — `src/store/appStore.ts:54`의 `isDirty`가 sticky(`|| s.isDirty`)여서 조합 글리프 삽입만으로 플래그가 latch되고 취소가 내용을 되돌려도 플래그는 복구되지 않는다. 이는 이 SPEC이 만든 것이 아니라 SPEC의 요구를 실행 가능하게 만들면서 드러난 기존 결함이며 **issue #12**로 등록되었다. 요구사항은 정책 의미론을 규정하고, 관측 불가는 그 결함의 결과다 — issue #12가 해소되면 이 분기가 관측 가능해진다. 게이트 불변식 자체는 유닛 계층에서 검증된다(AC-WS-020b 개정 기록).

**"디스크 내용과 조합 텍스트를 모두 반영"은 요구사항이 아니다**: 그것은 3-way 병합이며 §D에서 명시적으로 제외된다. 두 내용을 합치는 유일한 경로는 사용자가 배너의 "차이 보기"로 확인한 뒤 스스로 판단하는 것이다.
...
**보류 표시의 종료 (REQ-WS-021 연동)**: 조합이 종료되어 보류가 풀리면 이 표시는 사라**지고(shall)**, 정책 라우터의 결과로 대체**된다(shall)** — 배너(REQ-WS-027) 또는 조용한 반영(REQ-WS-024). 보류 표시가 **아무 후속 표면 없이 사라지는 경로는 없다(shall not)**: 보류가 풀렸는데 배너도 뜨지 않고 버퍼도 그대로라면 사용자는 외부 변경이 사라졌다고 오해한다. 보류 표시는 정책 결과로 **인계**되는 것이지 소멸하는 것이 아니다.
