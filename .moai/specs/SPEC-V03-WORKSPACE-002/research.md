...
| 등급 | 의미 | 이 SPEC의 해당 항목 |
|---|---|---|
| **기계 재현** | 실제 모듈을 구동해 결함을 관측했다 | 결함 A 즉시 오염(§7.3a-1), **결함 A dirty 경유 오염 + `pending` 오염(§7.3a-3)**, **게이트 detach latch(§7.3d)**, **저장 await 창(§9.8d)** ↑판 0.3.7, **sticky dirty — issue #12** ↑판 0.3.7 |
| **코드 직독** | 소스를 읽어 형태를 확인했고 실행하지 않았다 | 조합 플래그 공유(§7.3b), 사이드카 재바인딩 플러시(§9.7), pandoc 신뢰 우회(§9.6), 재바인딩 시 라우팅 키 미갱신(§9.9), **StrictMode에서의 React effect 실행 순서**(N3 / `design.md` §6.2a — 관측하지 않았다) |
| **정적 소스 사실** | 코드에 그렇게 쓰여 있다는 것만 확인했고 그 결과를 관측하지 않았다 | `getAllWindows()` 브로드캐스트가 **경로를 등록한 적 없는 창에 실제로 도달하는가**(§7.3a-2 main 절반) |

AC 본문이 각 결함의 등급을 명시하며(`acceptance.md` 표기 규약), **등급을 올려 적지 않는다.**

**판 0.3.7 승격 2건 — 근거**: M1 구현이 실제 `useFileMenuCommands` 모듈을 **preload 브리지만 대체해** 구동하며 저장 await 창과 sticky를 함께 관측했다. 증거 `.moai/state/verify/m1/red-0-awaitwindow-repro.log`:
...
**승격하지 않은 2건 — 왜 그대로인가** (M0·M1 구현자 양쪽이 조용한 승격을 하지 않겠다고 명시했고, 이 표가 그 약속의 기록이다):

- **`getAllWindows()` main 절반** — M0 구현자의 18개 테스트도, 오케스트레이터의 독립 4건도 **창을 두 개 띄우지 않았다.** 확인된 것은 소스가 전체 창에 브로드캐스트한다는 사실이고, 미등록 창이 그것을 실제로 받아 적용하는 것을 관측한 실행은 없다. **정적 소스 사실 등급을 유지한다** — `AC-PANEL-084`가 소스 스캔에서 멈추는 이유다.
- **StrictMode에서의 React effect 실행 순서** — N3이 확정한 "`:74`가 `:83`보다 먼저 돈다"는 React가 effect를 선언 순서로 실행한다는 규칙에서 **읽어낸 것**이며 StrictMode 이중 마운트에서 관측하지 않았다. **코드 직독 등급을 유지한다.**
...
2b. **정정 (2026-08-09)**: §7.3a-3(dirty 경유 오염)과 §7.3d(detach latch)는 **기계 재현되었다.** 초판 §10은 두 항목을 코드 직독 등급으로 기재했으나 감사 재현으로 승격되었다 — §10.0 등급 표에 반영했다. 그 시점에 **등급을 올린 것은 이 둘뿐이었다.**

2c. **추가 승격 (판 0.3.7, M1 구현)**: **저장 await 창(§9.8d)과 sticky dirty(issue #12)** 도 기계 재현되었다 — 실제 `useFileMenuCommands`를 preload 브리지만 대체해 구동. 증거 `.moai/state/verify/m1/red-0-awaitwindow-repro.log`. **여전히 코드 직독으로 남는 항목**: 조합 플래그 공유 / 사이드카 플러시 / pandoc / 재바인딩 / StrictMode effect 순서. **정적 소스 사실로 남는 항목**: `getAllWindows()` main 절반. 승격 사유와 미승격 사유는 §10.0에 함께 적었다.
