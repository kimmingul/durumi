...
>
> **결정 10건이 §A.2에 있다** (`grep -c "^#### OQ-" plan.md` → 10) — **확정 6건**(OQ-1·2·3·4·5·6) / **미해결 4건**(OQ-7·8·9·10). 이 SPEC은 혼자 확정하면 안 되는 아키텍처 결정을 갖고 있으므로, 미해결 항목은 임의 선택 없이 **후보·대가·잠정 권고** 형태로 남겨 둔다. 확정에는 외부 교차 검토(OQ-1·2·5·6) 또는 사용자 결정(OQ-3·4)이 선행했다.
>
...
### §A.2 결정 로그 — **확정 6건 (OQ-1·2·3·4·5·6)** / 미해결 4건 (OQ-7·8·9·10)
...
#### OQ-10 — extension 3항목의 층 귀속 — *판 0.3.6 전수 실측에서 파생* (M5 진입 전 확정 필요)
...
**왜 지금 생겼는가.** REQ-PANEL-042의 판정을 금지 목록에서 **allowlist**로 바꾸면서(공통층 ∪ 보조층 밖 항목 = 실패), 오늘 조립되는 최상위 항목이 **전부 어느 한 층에 분류되어 있어야** 판정이 성립하게 되었다. 실측 결과 `MarkdownEditor.tsx:88-147`의 최상위 **24항목** 중 `design.md` §5.2가 분류한 것은 **21항목**이었고 3항목이 어느 층에도 없었다. 초판의 금지 목록 형태에서는 이 3항목이 "부재를 단언받지 않는" 상태로 조용히 통과했으므로 드러나지 않았다.
...
**번복 조건**: 사용자가 보조 패널에서의 AI 보조를 명시적으로 원하면 `ghostTextExtension`은 후보 2로 뒤집힌다 — 그 경우 **세션 상한을 패널 종류별로 분리**하는 것이 함께 필요하다(공유 예산이 위 대가의 핵심이므로).

**M0~M4는 이 결정을 기다리지 않는다.** extension 조립 재구성은 M5의 산출물이다.
...
**M1~M2·M4·M6~M7은 이제 차단 해제되었다** — OQ-1(레이아웃)·OQ-2(상태 소유)·OQ-5(pathGuard)·OQ-6(저장 경로 + EOL)에 이어 **판 0.3.11에서 OQ-3(모드 컨트롤)·OQ-4(라우팅 배선)가 확정**되었다. 남은 미해결 4건의 시점은 아래와 같다.
...
| **OQ-10** extension 3항목의 층 귀속 | 미해결 (판 0.3.6 신설) | **M5 착수 전**. REQ-PANEL-042가 allowlist 판정으로 바뀌어 **24항목 전수 분류가 판정의 전제**가 되었다. `spellcheckExclusion`은 기술 근거로 이미 확정, 남은 것은 `ghostText`·`macroCompartment` 2건의 제품 판단 |
...
파일 종류 판정을 `shared/`에 순수 함수로 정의한다(B.2의 방향 뒤집기 적용). extension 조립을 3층으로 재구성하고 **최상위 항목 집합이 공통층 ∪ 보조층 안에 있음을 allowlist로 단언한다**(REQ-PANEL-042 — 금지 목록 열거가 아니다; **선행: OQ-10으로 24항목 전수 분류 확정**). `@codemirror/language-data`로 `.py`/`.json`/`.yaml`/`.yml` 문법을 조달하고 **`.csv`·`.bib`/`.bibtex`·미지 확장자는 평문 폴백한다**(판 0.3.6 실측 — 카탈로그에 `.csv`·`.bib` 부재). 열기 경로에 엄격 디코드를 적용해 바이너리를 편집 패널로 열지 않는다.
