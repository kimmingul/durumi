...
| 3 | `dirty-changed → isDirty: true` 선행 시 버퍼 불변 + `held-notify` | `shared/reconciliation.ts:193`의 `!state.isDirty`가 **유일한 보호막**임이 실행으로 확인됨. 깨끗한 문서만 취약하다 |
...
**함께 결정할 것**: 창 간 소유권 모델은 M0 이후에도 남는다. M0이 **데이터 손실**은 닫지만, 두 창이 같은 파일을 열었을 때 양쪽에 배너가 뜨는 표면 중복 같은 문제는 남는다 — v0.4 이후 별개 항목으로 둘 것인지 확인이 필요하다.
...
- **dirty 보호**: `!state.isDirty`(`shared/reconciliation.ts:193`)가 유일한 보호막임이 실행으로 확인되었다. 따라서 **AC는 깨끗한 문서 케이스를 반드시 다뤄야 한다** — dirty 케이스만 검사하면 결함이 통과한다.
