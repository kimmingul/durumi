...
**함께 기록하는 정정 하나 — 이 공백에 걸리는 것은 셋이지 넷이 아니다.** `AC-PANEL-036d`는 **Given이 "이 SPEC의 아티팩트"** 이고 `spec.md` REQ-PANEL-036c와 `design.md` §8.3b의 서술을 검사하는 **문서 게이트**다(판 0.3.1 D6에서 그 형태로 만들어졌다). 트리가 없어도 판정 가능하며 지금도 판정 가능하다. **배정은 바꾸지 않는다** — 트리 표면의 소유는 별도 위임으로 범위가 잡히는 중이므로, 여기서는 사실만 남긴다.
...
대상 요구: REQ-PANEL-**001**(M1에서 이관 — 판 0.3.6), 002, 003, 004, 005, 007, 036, 036b, 036c, 065
