...
>
> **결정 10건이 §A.2에 있다** (`grep -c "^#### OQ-" plan.md` → 10) — **확정 7건**(OQ-1·2·3·4·5·6·10) / **미해결 3건**(OQ-7·8·9). 이 SPEC은 혼자 확정하면 안 되는 아키텍처 결정을 갖고 있으므로, 미해결 항목은 임의 선택 없이 **후보·대가·잠정 권고** 형태로 남겨 둔다. 확정에는 외부 교차 검토(OQ-1·2·5·6) 또는 사용자 결정(OQ-3·4·10)이 선행했다.
>
...
### §A.2 결정 로그 — **확정 7건 (OQ-1·2·3·4·5·6·10)** / 미해결 3건 (OQ-7·8·9)
...
#### OQ-3 — 모드 모델: 컨트롤 표면과 `defaultMode` 의미론 · **확정: 후보 1 (활성 패널 종속 컨트롤 + `defaultMode`는 초기값만)**

**결정 (사용자)**: 상태바 라디오가 **활성 패널의 모드**를 표시·변경한다. **패널별 모드 변경은 prefs에 쓰지 않는다.** `prefs.editor.defaultMode`는 **새 원고 패널의 초기 모드로만** 쓴다.

**이 결정은 요구의 규범 내용을 바꾸지 않는다.** `spec.md` REQ-PANEL-021~024가 이미 후보 1의 형태로 쓰여 있다. 판 0.3.10까지 열려 있던 것은 "요구가 무엇을 말하는가"가 아니라 "세 후보 중 어느 것이 그 요구를 만족하는가"였고, 답이 후보 1이다. **새 요구도 새 AC도 생기지 않는다.**

**확정 논거**: `defaultMode`라는 이름 자체가 "기본값"이며, 기본값을 마지막 사용값으로 덮어쓰면 다중 패널에서 의미가 붕괴한다 — 마지막으로 모드를 바꾼 패널이 **다음 세션의 모든 패널**을 규정하게 된다. `design.md` §3의 취지(두 축을 한 저장 경로로 합치지 않는다)와 같은 축이다.

**(a) 질문 (판 0.1.0 원문 보존)**: 3-모드의 사용자 컨트롤 표면(상태바 라디오, 메뉴 모드 항목)을 어떻게 패널화하는가. `prefs.editor.defaultMode`를 언제 쓰는가. 보조 패널은 자기 표시 축(예: 줄바꿈 토글)을 갖는가.
...
**(b) 후보 — 기각 2건의 근거를 남긴다 (삭제하지 않는다)**

| # | 후보 | 판정 |
|---|---|---|
| 1 | **활성 패널 종속 컨트롤 + `defaultMode`는 초기값만** — 상태바 라디오가 활성 패널의 모드를 표시·변경. 패널별 모드 변경은 prefs에 쓰지 않고, `defaultMode`는 새 원고 패널의 초기 모드로만 쓴다 | ✅ **확정** |
| 2 | **활성 패널 종속 컨트롤 + 마지막 변경을 prefs에 반영** — 오늘 동작(`useMenuCommandRouter.ts:126, 131` — 실제 `:135`/`:140`, 아래 부수 정정)을 그대로 유지 | ❌ **기각** — 오늘 코드 변경이 가장 적다는 것은 **사실이고 그 값은 인정한다.** 그러나 그 싸기가 사는 것은 **이름과 어긋난 의미론**이다: 마지막으로 모드를 바꾼 패널이 전역 기본값을 결정해 다음 세션 모든 패널을 규정한다(REQ-PANEL-023 위반 소지) |
| 3 | **전역 단일 모드 유지** — 모드는 창 전역 1개. 패널별 독립을 포기 | ❌ **기각** — 구현이 최소인 것은 사실이나 REQ-PANEL-021을 정면으로 어긴다. 스크립트 패널을 Source로 보려고 원고 패널까지 Source가 된다 |

**(c) 대가 — 판 0.1.0 원문 보존. 후보 1의 비용은 과대 계상되어 있으며 아래 (d) 정정 A가 고친다**
...
**(d) 정정 A — 후보 1의 "모드 필드 이관"은 필드 신설이 아니라 이미 있는 필드를 잇는 일이다** (실측, HEAD `e9c5059`)

위 (c) 대가 표는 후보 1의 변경 파일에 `src/store/appStore.ts:65-71`(모드 필드 이관)을 적었다. **M1이 `PanelState.displayMode: EditMode`를 이미 만들어 두었으므로 그 항목은 신설이 아니다:**

| 지점 | 사실 |
|---|---|
| `src/store/workspaceStore.ts:102` | `readonly displayMode: EditMode;` — 필드 **존재** |
| `src/store/workspaceStore.ts:287` | 패널 생성 시 `'wysiwyg'`로만 설정 |
| `src/store/workspaceStore.ts:352` | 복원 시 `'wysiwyg'`로만 설정 |
| 읽는 곳 | **0곳** — 필드는 만들어졌으나 **죽어 있다** |

이는 `progress.md` M1 절이 "`displayMode` 필드는 만들되 배선하지 않았다"고 이미 기록한 그대로다. 따라서 M4의 작업은 **필드 신설이 아니라 배선**이며, 대가 표가 적은 것보다 싸다.

**그리고 §A.2가 적고 있지 않던 것이 하나 있다 — 모드 필드는 지금 둘이다.** `appStore.editMode`(살아 있음, **창 전역**)와 `PanelState.displayMode`(만들어졌으나 **죽어 있음**)가 공존한다. M4는 둘 중 하나를 없애는 것이 아니라 **소비처를 후자로 옮기는** 작업이고, 이 이중 상태 자체가 계획에 적혀 있지 않았다. 여기 적는다.

**부수 정정 — 줄 번호 2건이 밀렸다 (사실은 유지)**: 위 인용은 판 0.1.0 시점 트리를 가리킨다. M0~M3가 파일을 바꿨으므로 M4 착수 시 다음을 쓴다.

| 판 0.1.0 인용 | HEAD `e9c5059` 실제 |
|---|---|
| `src/store/appStore.ts:65-71` (모드 필드) | `:29` `editMode` / `:31` `lastNonMarkdownMode` / `:41` `setEditMode` |
| `src/hooks/useMenuCommandRouter.ts:126, 131` (prefs 쓰기) | `:135` (`toggleSourceMode` 후 `prefsSet`) / `:140` (`setEditMode` 후 `prefsSet`) |
| `src/hooks/usePreferencesInit.ts:58` | **정확히 일치** — 정정 불필요 |

**모드는 여전히 창 전역이다.** 줄이 밀렸을 뿐 후보 1이 고쳐야 할 대상은 그대로다.

**보조 패널의 별도 표시 축(줄바꿈 등)은 이 SPEC에서 도입하지 않는다** — 판 0.1.0의 권고를 확정으로 승격한다. 어느 요구사항도 요구하지 않으며 `spec.md` §D.6이 비마크다운 고급 편집 기능을 범위 밖으로 두었다.

**번복 조건 (이 결정이 틀리게 되는 경우)**: 사용자가 "모든 패널이 항상 같은 모드여야 한다"를 명시적으로 요구하면 후보 3이 옳아지고, 그때는 REQ-PANEL-021이 함께 바뀌어야 한다. **그 요구는 SPEC-2의 범위에 없다.**

---

#### OQ-4 — 포커스·라우팅 배선의 형태 · **확정: 후보 1 (활성 뷰 접근자 1개)**

**결정 (사용자)**: `panelStore`가 `getActiveView(): EditorView | null`을 제공하고, 훅들이 `RefObject` 대신 **접근자**를 받는다.

**확정 논거 — 낡은 ref 위험이 실재한다 (실측 확인, HEAD `e9c5059`)**: `src/hooks/useMenuCommandRouter.ts:107`의 `const view = editorViewRef.current`는 `async` 핸들러의 **첫 줄**이고, 그 아래로 `await` 분기가 여럿이다. 뷰가 하나뿐인 오늘은 무해하지만, 비동기 커맨드 처리 중 활성 패널이 바뀌면 **낡은 뷰에 작용한다.** 접근자 형태는 **사용 시점에** 활성 패널을 읽으므로 이 창이 닫힌다.

**(a) 질문 (판 0.1.0 원문 보존)**: `useMenuCommandRouter`와 그 형제 훅들이 캡처하는 단수 `editorViewRef`(`src/App.tsx:97, 102-103, 119`)를 어떤 형태로 대체하는가. 활성 패널의 정의는 `spec.md` REQ-PANEL-030이 확정했으므로 남은 것은 **배선 형태**다.

**(b) 후보 — 기각 2건의 근거를 남긴다 (삭제하지 않는다)**

| # | 후보 | 판정 |
|---|---|---|
| 1 | **활성 뷰 접근자 1개** — `panelStore`가 `getActiveView(): EditorView \| null`을 제공하고 훅들이 그것을 쓴다. 훅 시그니처는 `RefObject` 대신 접근자를 받는다 | ✅ **확정** |
| 2 | **ref 의미론 유지** — `editorViewRef`를 "활성 패널의 뷰를 가리키는 ref"로 재정의하고 활성 전환 시 `.current`를 갱신. 훅 시그니처 무변경 | ❌ **기각** — 변경이 최소인 것은 **사실이다.** 그러나 낡은 ref 창을 닫지 못한다: `.current`를 핸들러 진입 시 한 번 뽑는 형태가 그대로 남으므로, 확정 논거가 정면으로 겨냥한 위험이 그대로 살아남는다 |
| 3 | **각 훅에 패널 ID 인자 추가** — 호출부가 어느 패널을 대상으로 하는지 명시 | ❌ **기각** — 명시성은 최대이나, REQ-PANEL-030이 정의한 "활성 패널"을 **호출부마다 다시 계산**하게 만들어 단일 정의가 분산된다 |

**(c) 대가 — 판 0.1.0 원문 보존. "5개 훅"은 틀린 개수이며 아래 (d) 정정 B가 고친다**
...
| 압력받는 불변식 | 없음 | **미묘한 위험**: `useMenuCommandRouter.ts:98`(실제 `:107`)이 핸들러 진입 시 `.current`를 한 번 뽑아 쓰므로, 비동기 커맨드 처리 중 활성 패널이 바뀌면 낡은 뷰에 작용한다. 오늘은 뷰가 하나라 무해했다 | 없음. 대신 "활성 패널"이라는 개념이 호출부마다 반복 표현되어 REQ-PANEL-030의 단일 정의가 흐려진다 |
| 얻는 것 | 활성 패널 판정이 한 곳 | 변경 최소 | 명시성 최대 |

**(d) 정정 B — "5개 훅"은 4개다. 그리고 이것은 드리프트가 아니라 plan 작성 오류다** (실측, HEAD `e9c5059`)

위 (c) 대가 표는 후보 1의 변경 파일로 훅 **5개**를 열거하고, 후보 3의 깨질 테스트로 "위 5개 훅 테스트 전부"를 적었다. 실제로 `editorViewRef`를 받는 훅은 **4개**다:

| 훅 | `editorViewRef`를 받는가 |
|---|---|
| `src/hooks/useAiPalette.ts` | 예 |
| `src/hooks/useCitationInsertFlow.ts` | 예 |
| `src/hooks/useMenuCommandRouter.ts` | 예 |
| `src/hooks/usePickAndInsertImage.ts` | 예 |
| `src/hooks/useMemoCaretFocus.ts` | **아니다 — ref가 아니라 값을 받는다.** `src/App.tsx:114`가 `useMemoCaretFocus(editorView, content)`로 호출한다 |

**이것은 M0~M3가 만든 드리프트가 아니다.** `git show 040df4a:src/App.tsx`의 `:97`에서도 이미 `useMemoCaretFocus(editorView, content)`였다 — **plan을 쓸 때부터 틀린 개수다.** 즉 이 인용은 작성 시점에 트리와 대조된 적이 없다.

**인용된 앵커 자체가 오류의 자리였다.** (a) 질문이 적은 `src/App.tsx:97, 102-103, 119` 중 **선두 `:97`이 바로 그 값 전달 호출**이고, ref를 캡처하는 셋은 `:102-103`(citation·ai)과 `:119`(pickAndInsertImage)다. 앵커 넷을 훅 다섯으로 센 것도 아니고, **ref를 받지 않는 호출을 ref 캡처 목록의 첫 줄에 올린 것**이다. M4가 쓸 HEAD `e9c5059` 앵커:

| 훅 | 판 0.1.0 앵커 | HEAD `e9c5059` |
|---|---|---|
| `useCitationInsertFlow(editorViewRef)` | `App.tsx:102` | `:127` |
| `useAiPalette(editorViewRef)` | `App.tsx:103` | `:128` |
| `useMenuCommandRouter({ editorViewRef, … })` | `App.tsx:107` | `:132` |
| `usePickAndInsertImage(editorViewRef)` | `App.tsx:119` | `:144` |
| ~~`useMemoCaretFocus(editorView, content)`~~ | ~~`App.tsx:97`~~ | `:114` — **대상 아님(값 전달)** | 그 사실이 이 정정의 요점이며(§C.0a·판 0.3.6 R2와 같은 서명이다: 계획 자체에 한 번도 돌려본 적 없는 검사), 귀결은 **후보 1의 비용과 후보 3의 비용이 각각 한 파일씩 과대 계상되어 있었다**는 것이다.

**권고는 뒤집히지 않는다.** 후보 1의 근거는 훅 **개수**가 아니라 **낡은 ref 위험**이며, 그 위험은 위 확정 논거에서 실측으로 확인했다. 개수가 5에서 4로 줄어도 위험은 그대로다 — **비용 항목이 정정되었을 뿐 논거는 손대지 않는다.**

**부수 정정 — `useMenuCommandRouter`의 앵커가 9줄 밀렸다**: 판 0.1.0은 `:98`로 적었고 HEAD 실제는 `:107`이다. 구조는 동일하다(`async` 핸들러 첫 줄 + 그 아래 다중 `await` 분기). 같은 `:98`을 인용하는 곳이 둘 더 있고 처리가 다르다:

| 인용처 | 처리 | 이유 |
|---|---|---|
| `design.md` §4.4 | **함께 정정한다** | 설계 주장이므로 틀린 개수를 남기면 M4 구현자가 모순된 두 출처를 읽는다 |
| `research.md` §4.3 | **고치지 않는다** | plan 작성 시점의 **조사 스냅샷**이다. 관측 시점을 사후에 옮기면 기록이 아니게 된다 |

**함께 결정할 것 (판 0.1.0에서 승계, 유효)**: `research.md` §4.3의 6개 `durumi:*` 창 전역 이벤트에 발신 패널 식별자를 싣는 방식(REQ-PANEL-034). CustomEvent `detail`에 필드를 추가하는 것이 최소 변경이며, 이벤트 버스 제거는 CodeMirror 데코레이션 계층 재작성을 뜻해 범위를 넘는다(`design.md` §4.3).

**번복 조건 (이 결정이 틀리게 되는 경우)**: "활성 패널"이 여러 개로 쪼개지면(예: 사이드바 종류마다 다른 활성 대상) 접근자 1개로는 부족해지고 후보 3이 옳아진다. REQ-PANEL-030이 단일 정의를 고정하는 한 그 경우는 오지 않는다.
...
#### OQ-10 — **확정:** extension 3항목의 층 귀속 — *판 0.3.6 전수 실측에서 파생, 판 0.3.12에서 확정*

**결정 (판 0.3.12, 사용자 결정 2건).** `ghostTextExtension()` = **마크다운층**, `macroCompartment` = **공통층**. 둘 다 기술 제약이 아니라 **제품 판단**이었고 사용자가 확정했으며, **둘 다 아래 (d)의 잠정 권고와 일치한다**. `spellcheckExclusion()`은 lezer 노드 의존이라는 기술 근거로 이미 마크다운층 확정이었고 질문이 아니었다.

**귀결 — 3층 분류가 전수가 되었다.** 공통층 **11** + 마크다운층 **13** = **24**로 실측 최상위 항목 수와 일치한다(`design.md` §5.2). `REQ-PANEL-042`의 allowlist 집합이 비로소 정의되었고, `AC-PANEL-042`의 판정 불가 표시가 해제되었으며, **M5의 선행 조건이 해소되었다**(§A.3). 요구·AC·마일스톤 배정은 바뀌지 않았다 — 열려 있던 것은 "요구가 무엇을 말하는가"가 아니라 "allowlist 집합의 원소가 무엇인가"였다.

**확정을 뒷받침한 실측 2건** (판 0.3.12에 새로 측정, HEAD `6d2fd0c` 기준)

| 항목 | 실측 | 확정에서의 역할 |
|---|---|---|
| `ghostText` 세션 예산의 스코프 | `src/editor/ai/ghostText.ts:95`의 `let sessionTriggerCount = 0`은 **모듈 스코프**다 — `EditorView`별이 아니다. `:155`가 `aiCfg.ghostTextSessionCap ?? 100`과 대조하고 `:174`가 증가시킨다(기본 상한 100 — `electron/preferences.ts:95`) | (c) 표의 "원고와 공유하는 세션 호출 상한"이 **추정이 아니라 실측**임이 확인되었다. 공통층에 두면 보조 패널의 타이핑이 **창 전역 단일 예산**을 소모해 원고 작업을 침식한다 |
| `macroCompartment`의 마크다운 의존 | `src/editor/keymap/macros.ts:61-77` `buildMacroKeymap`은 순수 텍스트 삽입이다(`expandMacro` → `view.dispatch({changes, selection})`). 마크다운 의존 **0건** | (d)의 "배제할 기술적 이유가 없다"가 확인되었다 |

**과잉주장 하지 않는다** (규약 준수): ghostText가 보조 패널에서 **동작 불가한 것이 아니다.** 트리거 로직은 언어 무관이므로 기술적으로는 동작한다(아래 (d) 참조). 마크다운층 배치의 근거는 **제품 판단**(보조 패널은 저작 도구가 아니다)과 위 공유 예산 실측이 보여 준 **비용**이지, 불가능성이 아니다.

---

**아래는 결정에 이르기까지의 기록이다** — 질문·후보·대가·잠정 권고. 기각된 후보의 대가 분석은 다음 구현자가 읽는 논거이므로 남긴다.

**(a) 질문.** `ghostTextExtension()`과 `macroCompartment`(`buildMacroKeymap`)를 보조 패널에 로드하는가? (`spellcheckExclusion()`은 아래 근거로 이미 마크다운층으로 확정한다 — 질문이 아니다.)

**왜 지금 생겼는가.** REQ-PANEL-042의 판정을 금지 목록에서 **allowlist**로 바꾸면서(공통층 ∪ 보조층 밖 항목 = 실패), 오늘 조립되는 최상위 항목이 **전부 어느 한 층에 분류되어 있어야** 판정이 성립하게 되었다. 실측 결과 `extensions:` 배열의 최상위 **24항목** 중 `design.md` §5.2가 분류한 것은 **21항목**이었고 3항목이 어느 층에도 없었다. 초판의 금지 목록 형태에서는 이 3항목이 "부재를 단언받지 않는" 상태로 조용히 통과했으므로 드러나지 않았다.

> **인용 앵커는 커밋 기준이다** (판 0.3.6 ⑧ 교훈). 위 배열은 **`040df4a:88-147` = HEAD `6d2fd0c:91-150`**이다 — M0이 배열 **위쪽**에 `useEffect` 블록을 삽입해 줄 번호가 3줄 밀렸고, **배열 블록 자체는 두 커밋에서 바이트 동일**하다(`diff` 무출력, 판 0.3.12 실측). 판 0.3.6이 적은 `:88-147`은 그 시점(`040df4a`)에서는 정확했다.
...
| 항목 | 후보 1(마크다운층)의 대가 | 후보 2(공통층)의 대가 |
|---|---|---|
| `ghostTextExtension` | `.py`/`.bib` 패널에서 AI 고스트 텍스트가 없다 — 보조 패널을 "코드도 AI로 쓰는 곳"으로 기대한 사용자에게는 결손 | `.py` 패널의 타이핑이 **원고와 공유하는 세션 호출 상한**(`GHOST_TEXT_BOUNDS.sessionCap`)을 소모하고 LLM 비용이 발생한다. 프롬프트가 원고 전제로 만들어졌다면 보조 파일에서 품질이 낮다 |
| `macroCompartment` | 사용자 정의 매크로가 보조 패널에서 동작하지 않는다 — 사용자에게는 설명하기 어려운 비일관 | 마크다운 전제로 만든 매크로(예: 인용 스니펫)가 `.py` 패널에서 무의미한 텍스트를 삽입할 수 있다. 다만 그것은 **사용자가 만든 매크로의 문제**이지 셸의 결함이 아니다 |

**(d) 잠정 권고**

| 항목 | 권고 | 근거 |
|---|---|---|
| `spellcheckExclusion()` | **마크다운층 (확정)** | 질문이 아니다. `src/editor/spellcheckExclusion.ts`가 `syntaxTree`의 FencedCode·InlineCode·FrontMatter **lezer 노드**에 의존한다(파일 상단 주석이 대상 노드를 열거). 마크다운 언어층이 없으면 그 노드가 생기지 않아 기능이 성립하지 않는다 — **기술적 사실**이며 제품 판단이 아니다 |
| `ghostTextExtension()` | **마크다운층** | 트리거 로직 자체는 언어 무관이다(`src/editor/ai/ghostText.ts:118` 이하 — `docChanged`/`selectionSet`에만 반응). 따라서 이것은 **제품 판단**이지 기술 제약이 아니다. 보조 패널의 목적은 "원고 옆에서 스크립트·데이터를 보고 고치는 것"이지 저작이 아니며, 공유 예산 소모가 원고 작업을 침식하는 쪽의 손해가 더 크다 |
| `macroCompartment` | **공통층** | `src/editor/keymap/macros.ts:61-75`는 순수 텍스트 삽입이며 마크다운 의존이 없다. 배제할 기술적 이유가 없고, 배제하면 "왜 여기선 안 되나"라는 설명 부담만 남는다 |

**번복 조건**: 사용자가 보조 패널에서의 AI 보조를 명시적으로 원하면 `ghostTextExtension`은 후보 2로 뒤집힌다 — 그 경우 **세션 상한을 패널 종류별로 분리**하는 것이 함께 필요하다(공유 예산이 위 대가의 핵심이므로). **이 조건은 확정 이후에도 살아 있다** — 판 0.3.12의 실측이 공유 예산을 모듈 스코프 단일 카운터로 확인했으므로, 번복 시 필요한 작업이 무엇인지가 오히려 더 분명해졌다(`ghostText.ts:95`의 모듈 스코프 카운터를 패널 종류별로 쪼개는 일).

**어떤 마일스톤도 이 결정을 기다리지 않는다** (판 0.3.12 확정 이후). M0~M4는 애초에 무관했고, extension 조립 재구성은 M5의 산출물이며 M5의 선행 조건은 이제 해소되었다.
...
**M1~M2·M4~M7은 이제 차단 해제되었다** — OQ-1(레이아웃)·OQ-2(상태 소유)·OQ-5(pathGuard)·OQ-6(저장 경로 + EOL), **판 0.3.11의 OQ-3(모드 컨트롤)·OQ-4(라우팅 배선)**, 그리고 **판 0.3.12의 OQ-10(extension 층 귀속)**이 차례로 확정되었다. 남은 미해결 **3건**(OQ-7·8·9)의 시점은 아래와 같다.
...
| **OQ-3** 모드 컨트롤 | ✅ 확정 (후보 1 — `defaultMode`는 초기값만. 판 0.3.11) | M4 차단 해제. 요구·AC 무변경 — 정정 A는 비용 항목만 고친다(`displayMode`는 M1이 이미 만들었고 읽는 곳이 0곳) |
| **OQ-4** 라우팅 배선 | ✅ 확정 (후보 1 — 활성 뷰 접근자 1개. 판 0.3.11) | M4 차단 해제. 정정 B: "5개 훅"은 **4개**이며 이는 드리프트가 아니라 **plan 작성 오류**다. 논거(낡은 ref 위험)는 불변 |
| OQ-8 창 축 AC 범위 | 미해결 (범위 축소됨) | M3 착수 전. **핵 결함은 M0이 닫으므로 "창 2개 e2e를 추가할 것인가"만 남았다** |
| OQ-9 패널 배치 persist | 미해결 | M8 착수 전 |
| **OQ-10** extension 3항목의 층 귀속 | ✅ 확정 (ghostText = 마크다운층 / macro = 공통층. 판 0.3.12) | **M5 차단 해제.** 3층 분류가 전수(공통층 11 + 마크다운층 13 = 24)가 되어 REQ-PANEL-042의 allowlist 집합이 정의되었다. 둘 다 잠정 권고와 일치했고, 요구·AC·마일스톤 배정은 무변경 |
...
> **이 금지는 네 번의 시도 끝에 확정되었다 (D11 → N2 → R1).** 세 번의 철회가 모두 실패한 이유는 **철회한 문장을 지웠을 뿐 같은 허가를 다른 말로 주는 문장을 남겼기** 때문이다:
> `AC-PANEL-082`의 예외(D11에서 철회) → `plan.md` §E 7a의 "예외이며 … 별도 증거"(N2에서 철회) → **§A.5의 "등록 방식은 바뀐다 — arity 단언 대상이 아니다"(R1에서 철회)**.
> 마지막 것은 "예외"라는 단어를 쓰지 않았기 때문에 문구 기반 검색을 통과했다.
>
> **따라서 검색 질문이 바뀌어야 한다**: "철회된 **문장**이 남아 있는가"가 아니라 **"다섯 파일 중 하나를 손대도 된다고 말하는 문장이 어떤 표현으로든 있는가"**. 판 0.3.5의 확장 검색 결과는 `progress.md` §E.1c에 기록했다.
...
| `src/editor/applyExternalChange.ts` | `LAYER_FILES`. `applyExternalChange(target, nextContent)` **arity 2 고정**(`:171-174`). `target`이 이미 패널 식별자이므로 라우팅은 호출자가 target을 고르는 것으로 성립한다. **이 파일은 무변경이다 — 예외 없다.** REQ-PANEL-055는 **스토어의 `effectHandler`를 문서별로 만드는 것**으로 충족되며(`design.md` §6.2a), `registerReconciliationExecutor`는 주입된 setter를 호출하고 teardown 클로저를 반환할 뿐 모듈 수준 상태를 갖지 않는다. REQ-055가 제거해야 할 창 전역 싱글턴은 **스토어 쪽**이다 |
...
`design.md` §5.2의 층 구조를 그대로 구현한다. 조립 지점은 `MarkdownEditor.tsx:86-148`(`040df4a` 기준; HEAD `6d2fd0c`에서는 `:89-151`) **하나로 유지**한다 — 두 곳에서 조립하면 공통 항목 **11개**(판 0.3.12에서 `macroCompartment` 합류)가 드리프트하고, 그것이 `structure.md` §10이 기록한 `StyleSet` 중복과 같은 함정이다.
...
`@codemirror/language-data`는 이미 의존성이고(`package.json:39`) 오늘도 로드된다(`MarkdownEditor.tsx:8, 94`). 카탈로그의 `LanguageDescription`은 문법 본체를 지연 로드하므로 보조 패널을 열 때만 해당 언어 청크를 받는다. **카탈로그가 무엇을 덮는지는 실측했다** (판 0.3.6, 감사 지적 R7): `.py`/`.json`/`.yaml`/`.yml`은 있고 **`.csv`·`.bib`·`.bibtex`는 없다**. 후자는 REQ-PANEL-045의 평문 폴백이 정상 경로로 규정한다.
...
**세 번째 조항 (판 0.3.9 신설) — 판정 대상에는 생산자가 있어야 한다.** 어떤 AC가 판정하는 표면은 **어느 마일스톤인가가 그것을 생산해야** 하며, 판정만 배정하는 것으로는 부족하다. 위 두 조항은 "항목이 축에서 빠졌는가"를 보지만 이 조항은 **"판정 대상이 존재하게 되는가"** 를 본다 — 둘은 다른 실패를 잡는다.

기계 점검 두 가지 (마일스톤 진입 시):

1. **§A.4 전수 대조**: §A.4가 "프로덕션 호출부 0곳"으로 열거한 표면은 **어느 마일스톤의 `대상 요구:`에도 그것을 생산하게 만드는 요구로 등장해야 한다.** 산문에 적힌 것은 배정이 아니다.
2. **생산 요구 부재 탐지**: 한 마일스톤의 `대상 AC:`가 인용하는 요구가 **전부 제약형**(`shall not` / "…만 제시한다" / 향후 제안에 대한 구속)이고 **그 표면을 만들라는 요구가 하나도 없으면**, 그 표면에는 소유자가 없다.

**이 조항은 실제 사고에서 나왔다 (M2, `AC-PANEL-036 / 036b / 036c` 미시도)** — 진단은 아래 §C.0a.

**네 번째 조항 (판 0.3.10 신설) — 생산된 능력에는 도달 가능한 진입점이 있어야 한다.** 사용자가 호출하는 능력을 추가하는 마일스톤은 그 능력을 **호출 경로를 통과해 도달하는 AC를 최소 하나** 가져야 한다 — 메뉴 커맨드·단축키·렌더된 컨트롤 중 하나이며, **스토어를 직접 호출하는 것은 이 조항을 충족하지 않는다.**

> **AC가 전부 스토어를 직접 호출하면 진입점 부재가 구조적으로 보이지 않는다.** 커맨드 경로를 통과하는 단언이 마일스톤마다 최소 하나 있으면 이 계열이 다시 새지 않는다.

구현 예시: `tests/layout/panelCommands.test.tsx`. **세 사례 중 어느 것도 이 검사를 통과하지 못했을 것이다** — 그것이 이 조항을 조항으로 만드는 근거다.

**세 조항은 한 가족이다 — 세 개의 패치가 아니다.** 모두 **코드가 아니라 계획에 돌리는 검사**이며, 각각 사슬의 다른 고리를 본다:

| 조항 | 무엇을 보는가 | 낳은 사고 |
|---|---|---|
| N1 (판 0.3.3, 위 두 줄 규약) | 항목이 **축에 배정**되었는가 | AC 85건 미배정 — `대상 AC:` 줄이 M0에만 있었다 |
| 판 0.3.9 (세 번째) | 판정 대상에 **생산자**가 있는가 | 프로젝트 트리 표면 부재 — 제약만 있고 생산 요구가 없었다 |
| 판 0.3.10 (네 번째) | 생산된 능력에 **도달 가능한 진입점**이 있는가 | 분할 진입점 부재 — 스토어 액션만 있고 메뉴·단축키가 없었다 |

사슬로 읽으면 **배정 → 생산 → 도달**이다. 앞의 둘을 통과해도 마지막에서 샐 수 있다는 것이 세 번째 사례가 보여준 것이다.

> **번호에 대한 주석**: 오케스트레이터는 이것을 "세 번째 조항"이라 불렀는데, 위 표의 **가족 안에서는 세 번째**가 맞고 **§C.0의 조항 번호로는 네 번째**다(기본 규약의 두 문장 + 판 0.3.9). 두 셈이 다르므로 여기 적어 둔다.
...
#### M1~M8 배정은 **도출값이며 사람이 검토하지 않았다** — 반증 조건

`plan.md` §C의 M1~M8 `대상 AC:` 줄은 손으로 고른 것이 아니라 **AC → REQ(`↔` 마커) → 마일스톤(`대상 요구:`)** 을 조합한 **도출값**이다. 도출 규칙 자체는 검토되지 않았으므로 다음을 기록한다.

**도출이 서는 전제**: AC↔REQ 매핑이 정확하다는 것. iteration 3이 그것을 `orphan 0 / uncovered 0`으로 독립 검증했으므로 전제는 확인되었다. **확인되지 않은 것은 그 위의 규칙**, 즉 "AC는 자기가 검증하는 REQ의 마일스톤에 속한다"가 맞는가다.

**규칙의 알려진 구조적 편향**: 도출은 **REQ 소유권**을 키로 삼고 **AC의 실행 전제조건**을 보지 않는다. 따라서 어떤 AC가 REQ X(마일스톤 M에 배정)를 검증하면서 **실행에는 M보다 뒤에 만들어지는 표면을 요구**하면, 도출은 그것을 M에 배정하고 그 AC는 M에서 판정 불가가 된다.

**무엇이 이 규칙을 반증하는가** (다음 패스가 확인할 것):

| 관측 | 판정 |
|---|---|
| 어떤 AC의 Given/When이 **배정된 마일스톤이 아직 만들지 않는 표면**을 요구한다 (예: 상태 계층 마일스톤에 배정되었으나 렌더된 패널 UI를 요구) | 개별 오배정 — 그 쌍만 고친다 |
| 위 형태가 **여러 마일스톤에 걸쳐 반복**된다 | **규칙의 체계적 편향** — 도출 키를 "REQ 소유권"에서 "AC 실행 전제조건"으로 바꾸거나, 두 축을 분리해 `대상 AC:`와 별개로 "판정 가능 시점"을 기록해야 한다 |
| `↔`가 복수 REQ를 인용하는 AC가 **여러 마일스톤에 동시 배정**되어 어느 쪽에서도 완결되지 않는다 | 과배정 — 대표 마일스톤 선택 규칙이 필요하다 |
| 제약(C-N)만 인용하는 AC가 교차 항목 5건 밖에 있다 | 버킷 오분류 — §C.0의 교차 항목 표를 넓혀야 한다 |
| **어떤 REQ를 검증하는 AC가 전부 그 REQ의 마일스톤 밖에 있다** | **REQ 배정이 틀렸다 — AC가 아니다.** 그 REQ는 자기 마일스톤 안에서 판정 불가하므로, AC를 옮길 것이 아니라 REQ를 AC 쪽으로 옮기거나 REQ를 분할한다 |

**다섯 번째 조건이 왜 늦게 추가되었는가 — 축 자체가 빠져 있었다** (판 0.3.6 신설). 위 네 조건은 **전부 AC→마일스톤 변을 감사한다.** 도출이 참으로 전제하는 두 번째 전제 — **REQ가 옳은 마일스톤에 있다** — 를 감사하는 조건은 하나도 없었다. §C.0은 전제를 "AC↔REQ 매핑의 정확성" 하나로만 적었으나 실제 전제는 둘이다. REQ→마일스톤 배정은 판 0.3.2보다 앞서며 **한 번도 검토된 적이 없다.**

이 공백은 즉시 대가를 치렀다. R2의 최초 처방("AC-001을 옮겨라")은 **첫 번째 조건의 처방(그 쌍만 고친다)을 그대로 따른 것**이었고, 그 처방이 orphan을 만들었다 — 다섯 번째 조건이 없으면 첫 번째 조건은 **자기 처방으로 다른 결함을 만들 수 있다.**

**기계 점검 결과 (실측)**:

| 시점 | 다섯 번째 조건 위반 수 | 비고 |
|---|---|---|
| 판 0.3.4 (R2 적용 전) | **0** | 조건이 그 시점에는 공허했다 |
| 판 0.3.5 (AC만 이동 — 잘못된 처방) | **1** | `REQ-PANEL-001 @M1` → 유일 검증 AC `AC-PANEL-001 @M2` |
| 판 0.3.6 (REQ도 함께 이동) | **0** | 복구 |

점검 방법: `acceptance.md`의 `^### AC-PANEL-` 제목 줄에서 `↔` 이후의 `REQ-PANEL-\d+[a-z]?`를 뽑아 AC→REQ를, `plan.md`의 `^대상 요구:`/`^대상 AC:`에서 REQ→M / AC→M을 만들고, 각 REQ에 대해 "검증 AC 중 같은 마일스톤에 있는 것이 하나라도 있는가"를 판정한다. **공허하지 않은 조건임이 증명되었다** — 다음 편집 한 번이 곧바로 위반시켰을 조건이므로, 이미 알려진 사례를 재진술하는 조건보다 값이 크다.

**첫 적용에서 실제 사례가 나왔다 (R2)**: `AC-PANEL-001`과 `011`이 M1에 배정되어 있었으나 둘 다 렌더된 편집 표면을 요구한다 — **반증 조건 1행(개별 오배정)이 첫 적용에서 적중했다.** M2로 옮겼다.

**그 처방의 절반이 틀렸고, 판 0.3.6이 되돌렸다.** `REQ-PANEL-001`을 검증하는 AC는 `AC-PANEL-001` **하나뿐이다.** AC만 M2로 옮기면 M1은 **AC 0건인 요구**를 갖게 되어 자기 마일스톤 안에서 판정 불가가 된다 — 결함 하나를 다른 결함과 맞바꾼 것이다. 두 AC에 각각 같은 검사를 적용한 결과:

| 옮긴 AC | 그 AC의 REQ | 이동 후 그 REQ에 M1에 남는 AC | 조치 |
|---|---|---|---|
| `AC-PANEL-001` | `REQ-PANEL-001` | **없음** (유일 검증 AC였다) | **REQ-PANEL-001도 M2로 함께 이동** |
| `AC-PANEL-011` | `REQ-PANEL-011` | `AC-PANEL-011a` (M1 유지) | REQ 이동 불필요 — AC 이동만 유효 |

**`REQ-PANEL-001`을 M2로 옮기는 것이 옳고 분할이 아닌 이유**: 이 요구의 두 규범 조항이 **둘 다 M2를 가리킨다.** (a) "창은 하나 이상의 패널을 담아야 한다" — 이 ≥1 불변식을 **집행**하는 형제 요구 `REQ-PANEL-004`("패널 수를 0으로 만들 수 없다")가 **이미 M2에 있다**; 두 진술이 같은 마일스톤에 모이는 쪽이 응집도가 높다. (b) "표면 상태(캐럿·선택·스크롤·실행 취소)를 공유하지 않는다" — 렌더된 표면 없이는 관측 불가이며, 그것이 AC-001이 M2로 간 이유 그 자체다. **분할을 기각한 근거**: 분할하면 M1 쪽 절반을 판정할 **새 AC를 만들어야 한다** — 배정 결함을 고치려고 AC 집합을 키우는 것이며, 게다가 M1에 남을 스토어 축 내용("패널마다 자기 문서 바인딩")은 `REQ-PANEL-010`(소유 축 표)이 이미 규범으로 담고 있어 분할된 절반은 신설이 아니라 중복이 된다.

**M1은 작업을 잃지 않는다**: M1 산문은 여전히 패널 목록을 정의하고, `대상 요구:`에 010/011/011a/012/013/014/015가 남는다. 옮긴 것은 **판정의 자리**이지 구현의 자리가 아니다.

**원인이 도출 규칙이 아니라는 점이 중요하다**: 두 AC의 형제 여덟 개(002~007)는 전부 M2로 갔고, 001만 M1로 간 이유는 `↔ REQ-PANEL-001`이고 **REQ-001의 M1 배정이 판 0.3.2보다 앞서기 때문**이다. 즉 **도출이 결함을 만든 것이 아니라 이미 있던 결함을 드러냈다.** 배정 축을 만든 것의 가치가 여기 있다.

**개수에 대한 정직한 기록**: 오케스트레이터의 지시는 "네 건이 한 마일스톤에 몰렸다"였으나, **내가 M1의 AC 10건을 하나씩 Given/When으로 검사한 결과 표면을 요구하는 것은 2건(001·011)이었다.** 나머지 8건(010·010b·011a·011b·012·013·013b·014)은 스토어 계층 + 스텁/스파이로 판정 가능하다 — `011b`의 "두 패널이 열린다"는 패널 목록 길이로, `014`의 "패널 분할·닫기·활성 전환"은 상태 전이로 판정된다. **개수를 맞추기 위해 추가 이동하지 않았다.** 감사가 네 번째·세 번째 건을 식별했다면 그 근거를 받아 재검토한다.

**후속 감사가 실제로 돌려준 것은 세 번째·네 번째 건이 아니라 축 하나였다** — 표면 요구 AC를 더 찾은 것이 아니라, **REQ→마일스톤 변을 감사하는 조건이 없다**는 지적이었다(위 다섯 번째 조건). 2건이라는 내 측정은 유지된다.

**이미 알려진 실제 사례 하나**: `AC-PANEL-095`(e2e 셀렉터 이관)는 `↔ C-13`이라 REQ를 인용하지 않으므로 도출로는 교차 항목 버킷에 떨어졌다. **손으로 M4에 배정했다** — 패널 지목 수단과 같은 작업이기 때문이다(`design.md` §3.2a). 즉 도출 규칙은 최소 한 건에서 이미 사람의 판단으로 보정되었고, 그것이 위 네 번째 반증 조건이 실재함을 보여준다.

---

### §C.0a 계획 공백 — 판정 대상에 생산자가 없었다 (M2에서 드러남, 판 0.3.9 기록)

M2가 `AC-PANEL-036 / 036b / 036c`를 **시도하지 않았고, 이유는 일정이 아니다.** 그 셋이 판정하는 **프로젝트 트리 표면이 이 저장소 어디에도 없다.**

**실측** (커밋 `bd62b80` 기준):

| 항목 | 결과 |
|---|---|
| `window.api.projectDiscover`의 렌더러 호출부 | **0곳.** `src/` 전체에 없다. 존재하는 것은 계약(`shared/ipc-contract.ts:412`)·preload 브리지 정의(`electron/preload.ts:37`)·main 핸들러(`electron/ipc/project.ts`)뿐 |
| `resolveWatchScope` / `registerWatchScope`의 프로덕션 호출부 | **0곳.** 정의(`electron/watchScope.ts:37, 67`)와 테스트만 |
| 사이드바 `FileTree` | 존재하나 **워크스페이스 폴더 축**이며 매니페스트 소유의 프로젝트 축이 아니다 |

즉 SPEC-1이 프로젝트 발견과 감시 범위 계층을 만들었고 **둘 다 표면에 배선된 적이 없으며**, 이 계획은 그 표면의 존재를 전제하는 AC 셋을 M2에 배정했다.

**원인 — "몰랐다"가 아니다. §A.4가 이미 정확히 적고 있었다.** §A.4는 두 표면을 "프로덕션 호출부 0곳"으로 열거하고 **"이 SPEC의 산출물이다"**, "이 SPEC이 프로젝트 트리·패널을 만들면서 처음 배선한다"고 명시했다. 그런데:

- **그 배선을 의무화하는 요구가 §B에 없다.** REQ-PANEL-036은 "새로고침 어포던스는 **프로젝트 트리 표면에 배치된다**", 036b는 "트리는 **열 수 있는 경로만 제시한다**", 036c는 향후 제안에 대한 구속이다 — **셋 다 트리에 걸리는 제약이며, 트리를 만들라는 요구가 아니다.** "셸은 프로젝트 트리를 제시해야 한다"에 해당하는 긍정 능력 요구가 존재하지 않는다.
- M2 산문은 "프로젝트 트리 표면의 골격과 수동 새로고침 어포던스를 배치한다"고 적었으나 **산문은 `대상 요구:`가 아니고 판정되지 않는다.**

**따라서 이 공백의 정확한 형태는 "아무도 배정받지 않았다"보다 좁다**: 계획이 그 표면을 **제약으로만 표현하고 생산 의무로는 한 번도 표현하지 않았다.** 배경 서술(§A.4)과 마일스톤 산문에는 있었고, 규범과 배정 축에는 없었다.

**N1과의 관계 — 형제이되 서명이 다르다.** N1은 **축은 있는데 항목이 빠진** 경우였고(AC 85건 미배정), 이것은 **항목은 배정되었는데 그 항목이 판정할 대상에 소유자가 없는** 경우다. 둘 다 **계획 자체에 한 번도 돌려본 적 없는 검사**에서 나왔다. §C.0 세 번째 조항이 이 서명을 잡는다.

**구현자가 부분 트리를 만들지 않은 판단은 옳다 — 이유를 기록한다.** 불완전한 신뢰 필터는 **OQ-5가 기각한 것을 정확히 생산한다** — 클릭하면 `PathNotAllowedError`가 나는 대상을 제시하는 트리다. 그것은 트리가 없는 것보다 **나쁘다**: 사용자에게 "이 트리는 믿을 수 있다"고 가르쳐 놓고 배신하기 때문이다. 신뢰 표면에서 부분 구현은 중립이 아니라 음수다.

**세 번째 사례 — 패널 분할 진입점 부재 (사용자가 실행 중인 앱에서 발견, 판 0.3.10).** 감사가 아니라 **사용자가 화면에서** 잡았다: 패널을 분할할 방법이 없었다. `workspaceStore`에 `openInNewPanel` / `closePanel` / `setActivePanel`이 있었으나 `src/`·`shared/`·`electron/` 전체에 **UI 호출부 0곳** — 메뉴 항목도 단축키도 버튼도 없었다. **테스트 2,256개가 초록인 채로 기능에 도달 불가**였는데, **M2의 모든 AC가 스토어 액션을 직접 호출**했기 때문이다.

형태는 위 두 사례와 같다: M2 산문은 "분할·닫기·공간 재배분을 **구현한다**"고 적었고, 요구는 제약형이며(`REQ-PANEL-005`는 "요청**하면** … 수행한다", `REQ-PANEL-004`는 "마지막 패널은 닫을 수 없다"), **"셸은 분할을 요청할 수단을 제공해야 한다"는 긍정 능력 요구가 없었다.** 산문은 `대상 요구:`가 아니고 판정되지 않는다.

해소: **`REQ-PANEL-005a`**(진입점) + **`AC-PANEL-005a`**(분할 — 호출 경로 판정) + **`AC-PANEL-005b`**(마지막 패널 닫기 무동작) 신설. 진입점은 이미 구현되어 있다(보기 메뉴 + `CommandOrControl+Alt+\` / `CommandOrControl+Alt+W`, `MenuCommand` → `electron/menu.ts:249-250` → `useMenuCommandRouter.ts:144` → `shared/menuLabels.ts:75, :190`). **§C.0 네 번째 조항이 이 사례에서 나왔다.**

**함께 기록하는 정정 하나 — 프로젝트 트리 공백에 걸리는 것은 셋이지 넷이 아니다.** `AC-PANEL-036d`는 **Given이 "이 SPEC의 아티팩트"** 이고 `spec.md` REQ-PANEL-036c와 `design.md` §8.3b의 서술을 검사하는 **문서 게이트**다(판 0.3.1 D6에서 그 형태로 만들어졌다). 트리가 없어도 판정 가능하며 지금도 판정 가능하다. **배정은 바꾸지 않는다** — 트리 표면의 소유는 별도 위임으로 범위가 잡히는 중이므로, 여기서는 사실만 남긴다.
...
대상 요구: REQ-PANEL-010, 011, 011a, 012, 013, 014, 015
대상 AC: AC-PANEL-010 / 010b / 011a / 011b / 012 / 013 / 013b / 014
> **AC-PANEL-001과 011은 M2로 이동했다 (감사 지적 R2)** — 둘 다 렌더된 편집 표면을 요구하는데 M1의 밀폐성 선언은 "편집 표면 없이 검증 가능"이다. 001은 캐럿 이동·선택·스크롤·실행 취소를, 011은 `EditorView` 인스턴스 수와 오류 표시 부재를 단언한다. **AC-013은 이동하지 않는다** — 폐기 확인은 스토어 + 스파이로 판정 가능하다(감사가 후보에서 철회).
> **`REQ-PANEL-001`도 함께 M2로 이동했다 (판 0.3.6, 다섯 번째 반증 조건)** — `AC-PANEL-001`이 그 요구의 **유일한** 검증 AC였으므로 AC만 옮기면 M1에 AC 0건인 요구가 남는다. 근거와 분할 기각 사유는 §C.0. **`REQ-PANEL-011`은 이동하지 않는다** — `AC-PANEL-011a`가 M1에 남아 판정 가능하다.
> M1이 패널 목록을 정의하는 작업 자체는 변하지 않는다. 이동한 것은 **판정의 자리**다.
...
대상 요구: REQ-PANEL-**001**(M1에서 이관 — 판 0.3.6), 002, 003, 004, 005, **005a**(진입점 — 판 0.3.10 신설), 007, 036, 036b, 036c, 065
대상 AC: AC-PANEL-**001** / 002 / 002b / 002c / 003 / 003b / 004 / 005 / **005a** / **005b** / 007 / **011** / 036 / 036b / 036c / 036d / 065
> **신규 3건의 배정은 §C.0 (b)의 의무이지 마일스톤 재구성이 아니다** — "요구나 AC를 새로 만들면 선언과 동시에 배정한다". `REQ-PANEL-005a`는 `REQ-PANEL-005`의 형제이고 진입점은 이미 M2 범위에서 구현되었으므로(`48ffc7d`) M2가 유일하게 맞는 자리다. **기존 항목의 배정은 하나도 바뀌지 않았다.**
...
**선행 조건 없음 — OQ-3·OQ-4는 판 0.3.11에서 확정되었다.** 모드는 **후보 1**(활성 패널 종속 컨트롤 + `defaultMode`는 새 원고 패널의 초기값만, prefs 쓰기 없음), 라우팅은 **후보 1**(`panelStore.getActiveView(): EditorView | null` 접근자 1개, 훅은 `RefObject` 대신 접근자를 받는다)이다. 착수 전 `plan.md` §A.2 OQ-3·OQ-4의 **정정 A·B를 읽을 것** — 대가 표의 비용 항목 두 곳이 과대 계상되어 있었다(`displayMode`는 이미 존재하며 읽는 곳이 0곳 / `editorViewRef`를 받는 훅은 5개가 아니라 **4개**).
...
**선행 조건 없음 — OQ-10은 판 0.3.12에서 확정되었다.** `ghostTextExtension` = 마크다운층, `macroCompartment` = 공통층(사용자 결정, 둘 다 잠정 권고와 일치). 이로써 3층 분류가 전수(**공통층 11 + 마크다운층 13 = 24**)가 되었고 **allowlist 집합이 정의되었다** — `design.md` §5.2가 그 집합의 정본이다.

파일 종류 판정을 `shared/`에 순수 함수로 정의한다(B.2의 방향 뒤집기 적용). extension 조립을 3층으로 재구성하고 **최상위 항목 집합이 공통층 ∪ 보조층 안에 있음을 allowlist로 단언한다**(REQ-PANEL-042 — 금지 목록 열거가 아니다). `@codemirror/language-data`로 `.py`/`.json`/`.yaml`/`.yml` 문법을 조달하고 **`.csv`·`.bib`/`.bibtex`·미지 확장자는 평문 폴백한다**. 열기 경로에 엄격 디코드를 적용해 바이너리를 편집 패널로 열지 않는다.

**문법 조달 재측정 (판 0.3.12 — `design.md` §5.3이 "M5 진입 시 재측정한다"고 지시한 것).** 설치본 `@codemirror/language-data` **6.5.2**, 카탈로그 **143개 언어**. 확장자 조회: `.py`→Python · `.json`→JSON · `.yaml`/`.yml`→YAML · `.csv`·`.bib`·`.bibtex`·`.txt`→**부재**(카탈로그 이름+확장자 전체에서 "bib" 문자열 0건). **판 0.3.6 실측과 변화가 없다** — REQ-PANEL-041(조달)과 REQ-PANEL-045(폴백)의 전제가 그대로 유효하며, 요구를 고칠 이유가 없다.
