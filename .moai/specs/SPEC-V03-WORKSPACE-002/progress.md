...
| N11~N15 | Low | (감사 보고서의 나머지 Medium/Low — 다음 패스에서 원문과 대조해 채운다) |

**N8은 다음 패스에서 우선 처리를 권고한다** — D13과 동일한 결함 형태가 새로 만든 AC에서 재발한 것이므로, 밀폐 범위를 AC 작성 시 기계적으로 점검하는 규약이 필요하다는 신호다. **N11~N15는 항목명을 복원해야 한다** — 이번 지시에 번호만 있고 내용이 없어, D7과 같은 복원 불가 상태를 만들지 않으려면 다음 패스 시작 시 감사 원문에서 옮겨 적어야 한다.
