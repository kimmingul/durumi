...
### M4-1 종료 — 오케스트레이터 독립 검증 + 플래키 1건 적출 (2026-08-11)

증거: `.moai/state/verify/m4-1-orch/`.

| 검사 | 결과 |
|---|---|
| `pnpm test` | exit 0 — **218 파일 / 2390 건** (기준선 211/2319 → +7 파일 / +71 건, 위임 보고의 증감 대조와 일치) |
| `pnpm typecheck` / `pnpm lint` | 둘 다 exit 0 |
| `pnpm test:coverage` | **96.29%** (기준선 96.24% → 상승) |
| 동결 7경로 + 보존 5(`editMode.ts`·그 테스트 포함) | 전부 `git diff --quiet 040df4a` exit 0 |
| `MarkdownEditor.tsx` | M4-1 구간(`fa97d20..HEAD`) 무접촉 |

**`defaultMode` 의미론 전환 확인**: 패널 모드 변경이 prefs를 덮어쓰는 경로 **0건**. `defaultMode`를 읽는 곳은 부트(`usePreferencesInit.ts:58`)와 새 원고 패널 초기값(`useMenuCommandRouter.ts:190`) 둘뿐 — OQ-3 후보 1이 정의한 그대로다. `SettingsDialog`의 쓰기는 **사용자가 기본값을 명시적으로 설정하는 경로**이므로 기각된 후보 2("마지막 패널 변경이 전역 기본값을 규정")와 다르다. 위임 에이전트가 지시에 없던 세 번째 쓰기(`StatusBar.tsx:92`)까지 찾아 제거했다.

**`displayMode` 처리 — 합치지 않고 역할을 갈랐다**: `PanelState.displayMode`는 살아 있는 값(편집 표면·상태바가 읽는다), `appStore.editMode`는 `defaultMode`로 개명되어 새 원고 패널 초기값만 정한다. 둘은 사본이 아니다 — 전자는 "지금 이 패널이 보여주는 것", 후자는 "세션을 가로지르는 선호"(출처는 `prefs`)다. 합치면 마지막으로 바꾼 패널이 다음 세션 전부를 규정해 **기각된 후보 2가 된다.**

#### 적출 — `AC-WS-020b`가 약 1/3 확률로 실패한다 (M4-1 소산이 아니다)

오케스트레이터의 전체 e2e 실행에서 `e2e/reconciliation-ime.spec.ts:202`(`AC-WS-020b` 취소된 조합)가 **1건 실패**했다. 위임 에이전트는 같은 스위트를 `0 failed`로 보고했고 **둘 다 참이다** — 이 테스트가 결정론적이지 않기 때문이다.

**실패 형태는 타임아웃이 아니라 의미론 단언이다**: 조합이 열려 있는데 `held-composition`이 아니라 **`held-notify`** 가 나왔다. 즉 **조합 보류가 걸리지 않았다.**

**양쪽을 같은 방법으로 재서 귀속을 확정했다** (신선 빌드, 스펙 단독 3회씩):

| 지점 | 3회 결과 |
|---|---|
| HEAD `eca4453` (M4-1 포함) | 6/6 · **1 failed** · 6/6 |
| `fa97d20` (M4-1 직전, 워크트리 신선 빌드) | **1 failed** · 6/6 · 6/6 |

**M4-1이 만든 것이 아니다.** 같은 비율로 흔들린다.

**이 적출은 앞선 기록도 정정한다.** IME 회귀 수정(`2221706`) 검증에서 이 문서가 적은 `213 passed / 0 failed`는 **운 좋게 통과한 실행**이었다. 스위트가 결정론적으로 초록인 것은 아니다 — 그때 이 테스트가 통과한 것이 관측 사실이고, 항상 통과한다는 것은 관측되지 않았다.

**무시할 수 없는 이유**: 실패는 조합이 열린 동안 외부 변경이 `held-notify`로 새는 것이다. 결정론적이라면 미저장 조합 텍스트가 덮어써질 수 있는 경로이고 IME 안전은 이 제품의 최우선 축이다. **다만 테스트 레이스인지(조합 등록 전파가 파일 쓰기보다 늦다) 실제 간헐 결함인지는 아직 모른다** — 둘을 가르는 관측을 하지 않았다. 사용자 결정으로 **M4-2 착수 전에 진단**한다: M4-2가 같은 조합 게이트를 문서 축으로 확장하므로(REQ-PANEL-054) 흔들리는 토대 위에 쌓게 된다.

#### 진단 결과 — **테스트 레이스**다. 단, 의심한 메커니즘과 다르다 (`b70f25a`)

**판정: (A) 테스트 레이스.** 게이트 누수가 아니다. 다만 원안 가설("조합 등록 전파가 파일 쓰기보다 늦다")은 **기각**되었다.

**참인 메커니즘은 조합의 *수명*이다.** `held-composition`은 "조합 중"을 전제로만 참일 수 있는 상태인데, 테스트가 그 전제를 검증하지 않고 결과만 단언했다. 조합이 환경 이벤트로 조기 종료되면 조정 계층은 보류분을 배너로 인계하고 `held-notify`가 되는데 — **그것은 `AC-WS-020`이 요구하는 정상 동작이다**(`shared/reconciliation.ts:221-228`의 `composition-end` 분기). 전제 없이 결과만 보면 **REQ-WS-028 계열 결함과 환경 잡음이 같은 빨간불**이 된다.

**세 관측이 서로를 보강한다**:

1. **타이밍 실측 (원안 기각)** — 편집 표면의 `compositionstart`는 `startComposition` 반환보다 **먼저** 도달한다(+3~4ms, 24/24 고정). 외부 변경이 표면에 뜨는 것은 **+420ms**(디바운스 200ms + IPC + 재검사 + 렌더). 여유 ~415ms이므로 "등록이 늦어서"로는 설명되지 않는다.
2. **조정과 무관한 자연 실패** — 전체 e2e 1회 실행에서 `e2e/composition-primitive.spec.ts:70`(P1)이 대신 떨어졌다: *"중간 await 동안 조합이 조기 종료됐다"*. **P1에는 외부 파일 쓰기도 조정 계층도 없다** — 조합을 열고 650ms 기다리는 것이 전부다. 조합 수명은 조정 코드의 함수가 아니다.
3. **결정론적 재현** — `AC-WS-020b`의 흐름에 `BrowserWindow.blur()` 하나만 주입하면 `held-notify` + `ends=1`이 나온다(보고된 실패와 동일). **그리고 `bufHasExternal=false`** — 외부 내용은 버퍼에 들어가지 않았다. **이 경로에 데이터 손실은 없다.**

**실패율 (정직한 수치)**: 위임 에이전트 기계에서 수정 전 **43회 중 0회 실패**(단독 20 + 파일 12 + CPU 부하 10 + 전체 1). 오케스트레이터 기계에서는 **6회 중 2회**. **둘 다 참이다** — 이 실패는 기계 상태에 달려 있다. 에이전트는 43회 무실패를 "고쳤다"의 근거로 쓰지 않았다고 명시했다. 근거는 blur 재현과 P1 실패다.

**수정 (`e2e/reconciliation-ime.spec.ts` 한 파일, +54/−2, 제품 코드 무변경)**: `AC-WS-019`가 **이미 쓰던 관문**을 `AC-WS-020b`·`AC-WS-023c`로 넓혔다 — `starts >= 1`(조합이 표면에 도달했는가) + `ends === 0`(**단언 시점에 아직 열려 있는가**). 그리고 고정 수면 `waitForTimeout(900)`을 관측 가능한 조건 대기 `waitForReconcileSurface`로 바꿔 노출 창을 900ms → ~420ms로 줄였다. **약화가 아니다**: `.skip`·재시도 래퍼·`DEBOUNCE_SETTLE_MS` 상향 어느 것도 없고 대기는 오히려 **줄었으며**, `held-composition` 단언은 구속력을 그대로 유지한다 — 관문은 단언을 **추가**할 뿐이다.

**진단이 반증 가능하게 남았다**: 앞으로 `starts >= 1`이고 `ends === 0`인데 `held-notify`가 나오는 실행이 있으면 그것이 (B) 제품 결함이며, 이번 관문이 정확히 그 조건을 분리해 낸다. **반증 도구가 곧 이번 수정이다.**

**남은 위험**: 노출 창은 **줄었을 뿐 사라지지 않았다**. 남은 ~420ms 안에서 환경 이벤트가 터지면 여전히 빨간불이나, 이제 `조합이 조기 종료됐다 — 게이트 누수가 아니라 조합 수명이 원인`으로 보고된다. 또 `waitForReconcileSurface`는 표면이 영영 안 뜨는 경우(무성 자동 반영 = 진짜 위험한 경로) 조용히 넘어가고 호출부가 `null`로 떨어진다 — 올바르지만 그 경로의 실패 **모양**이 바뀌었다.

**오케스트레이터 독립 검증**: 변경 표면이 `e2e/reconciliation-ime.spec.ts` 한 파일임을 확인(제품 코드 무접촉), 동결 7경로 `git diff --quiet 040df4a` exit 0, 추가된 줄에 `.skip`/`.fixme`/`.only`/타임아웃 상향 **0건**, 원격 `0 0`, 트리 clean.
