...
#### 다만 **`filePath` 변화만으로는 부족하다** — 뷰 준비 상태와의 결합 (감사 지적 N3)

위 코드 블록에는 **자체 가드가 있다**: `const view = viewRef.current; if (view) …`. React는 effect를 **선언 순서로** 실행하므로 **마운트 시 `:74` effect가 `:83` 마운트 effect보다 먼저 돌고**, `viewRef.current`는 `:151`에서야 설정된다 — 즉 **첫 실행에서 가드가 걸려 몸통을 건너뛴다.** `:166`이 `setDocPath`를 따로 시드하는 이유가 정확히 이것이며 `:165` 주석이 그렇게 적는다.

**오늘은 무해하다**: `src/App.tsx:153-160`이 무조건 렌더하고 `appStore.filePath`가 `null`로 시작하므로, 마운트 시 건너뛰는 것이 오히려 REQ-PANEL-070b(null-path는 등록하지 않는다)와 **일치한다.** 파일을 열면 `filePath` 변화로 등록된다.
