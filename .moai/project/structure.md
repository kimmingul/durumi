...
src/                렌더러(React 18 + CodeMirror 6) — 172개 파일, 약 29,558줄
├── App.tsx          얇은 셸 — 동작은 21개 훅으로 위임
├── main.tsx         React 루트
├── editor/           에디터 확장 계층 (§5 참고)
...
tests/               Vitest 유닛 테스트 169개 파일 (§7)
e2e/                 Playwright Electron 테스트 31개 spec (§7)
docs/                사용자·엔지니어링 문서 14개 (§8)
build/               앱 아이콘 (icon.svg / icon.png)
```

새 기능을 추가할 때 어느 디렉터리를 건드려야 하는지는 §9(실용 가이드)를 참고하라.
...
- **invoke 채널** — 렌더러가 요청하고 main이 응답하는 request/response 형태. `IpcApi`에 `Promise<...>`를 반환하는 메서드로 선언된다(약 70개 이상).
...
`electron/ipc.ts`(39줄)는 로직을 담지 않고 아래 10개 모듈의 등록 함수를 조립만 한다(v0.2 하드닝 스펙에 따라 각 모듈은 200줄 이하를 목표로 함).

| 모듈 | 담당 도메인 |
|---|---|
| `_shared.ts` | 공통 헬퍼 (메모 등 여러 도메인이 공유하는 로직) |
...
1. **파서** — `markdown({ base: markdownLanguage, codeLanguages: lezerLangs, extensions: [...] })`. Durumi 고유 문법(`[@key]` 인용, `%%memo%%`, CriticMarkup, `[toc]`, `==하이라이트==`/`~하첨자~`/`^상첨자^`, YAML front matter, 각주)은 전부 `src/editor/markdownExt/`의 커스텀 Lezer 확장(12개 파일)으로 구현된다. (`MarkdownEditor.tsx:85-101`)
...
| `store/` (8) | zustand 전역 상태 (§8 참고) | 새 전역 상태 슬라이스 |
...
**예시 — 새 마크다운 구문 하나를 추가하려면**: `markdownExt/`(파서) → `decorations/`(렌더링) → 필요시 `keymap/`(단축키) → `tests/editor/`(유닛) → IME에 영향을 준다면 `e2e/`에 `composeKorean` 기반 spec(§7 아래) 순으로 건드린다.
...
- **`Macro` 인터페이스 중복**: `shared/ipc-contract.ts:8-12`와 `electron/macros.ts:5` 양쪽에 별도로 `Macro` 인터페이스가 존재한다. `StyleSet`과 달리 이 쪽은 lockstep을 강제하는 테스트가 없다 — 향후 드리프트 위험이 있다(아래 findings 참고).
...
- **`^sup^`/인라인 코드는 원자적 마크 목록에 이름만 있고 미구현**: 명명은 되어 있으나 실제 atomic-mark 대상에는 포함되지 않는다(코드 확인 필요 시 `atomicInlineMarks.ts`의 `InlineMarkSpec` 목록 참고).

## 11. 테스트 조직

```
tests/                 Vitest 유닛 — 169개 파일
├── electron/ (29)      main 프로세스 로직
├── editor/   (34)      CodeMirror 확장, 데코레이션, 원자성
├── export/   (17)      내보내기 파이프라인
├── shared/   (16)      서지 파싱, 인용, 메모 등 순수 로직
├── sidebar/  (14)      사이드바 컴포넌트
├── utils/    (5)
└── hooks/, components/, i18n/, styles/
