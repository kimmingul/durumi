...
- **`prefs:set` 값 도메인 검증** (해소: `4ad2e30`): 경로 필드는 `assertPrefsPatchAllowed()`(`electron/pathGuard.ts:183-215`, §4)가 신뢰 경계를 지키고, 숫자 범위·enum 멤버십은 `shared/prefsValidation.ts`의 `sanitizePreferencesPatch()`가 담당한다. 두 계층은 목적이 다르다 — 전자는 경로 트래버설 방어, 후자는 입력 검증. 검증 호출은 IPC 핸들러가 아니라 `setPreferences()`(`electron/preferences.ts`) 데이터 계층에 있어 렌더러 경로뿐 아니라 모든 호출자가 보호된다. enum 위반은 drop(기존 저장값 유지), 숫자 범위 위반은 clamp한다.
- **렌더러 에러 경계·통합 에러 채널** (해소: `4a31696`): `src/components/ErrorBoundary.tsx`가 렌더 트리 throw를 잡아 빈 창 대신 복구 가능한 폴백을 그리고, `src/utils/errorSurface.ts`가 가드되지 않은 `unhandledrejection`(main에서 throw된 `PathNotAllowedError` 등)을 토스트로 흘린다. 배선은 `src/main.tsx` — 전역 그물은 렌더 트리 밖에 1회, 경계는 `LanguageProvider` 안쪽(폴백이 `t()`를 쓴다).
