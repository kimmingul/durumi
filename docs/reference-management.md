...
이 문서는 v0.1.10 기준의 실제 사용법을 한곳에 모아둔 안내서입니다.
...
1. **참고문헌 메뉴 → DOI로 참고문헌 추가…** 또는 단축키
2. DOI 입력 (`10.1056/NEJMoa2117417`, `https://doi.org/...` 형태 모두 OK)
3. **조회** 클릭 → Crossref에서 메타데이터 페치
4. 저자/연도/제목/저널 미리보기 확인
5. (선택) **☐ 본문에도 [@key] 삽입** 체크 — v0.1.10부터 기본 OFF.
   체크하면 캐럿 위치에 `[@key]`도 함께 넣어줍니다. 기본값은
   설정 `prefs.bibliography.insertCitationOnAdd`로 바꿀 수 있습니다.
6. **삽입** 클릭

결과:
- `references.bib`에 엔트리 추가
- `prefs.bibliography.autoSaveAbstract`(기본 ON)이면 `reference/<key>.md`에
  Crossref `abstract`(없으면 메타데이터 stub)를 자동 저장.
- DOI가 이미 라이브러리에 있으면 **추가가 거부**되고 우측 사이드바에서
  기존 항목 행이 1.5초간 노란색으로 강조됩니다.

DOI가 없는 엔트리는 **제목 + 첫저자 성 + 연도** 약한 매칭을 수행하고,
중복일 가능성이 있으면 "그래도 추가하시겠습니까?" 확인 모달을
띄웁니다.
...
1. **우측 사이드바 → 참고문헌 탭** (없으면 `Cmd/Ctrl + Shift + \\`로
   오른쪽 사이드바 토글 — references 탭이 마지막 활성 탭이었다면
   바로 열립니다)
2. 상단 검색 입력란에 검색어
3. 좌측 드롭다운으로 출처 선택 (Crossref / PubMed / KoreaMed)
4. 300ms debounce 후 결과 카드 표시
5. 원하는 결과의 **추가** 버튼 → bib에만 등록 (v0.1.10부터 기본 동작).
   `Shift`-클릭하면 **추가 + 캐럿 위치에 `[@key]` 삽입**까지 한번에.
   카드 하단에 힌트가 표시됩니다.
...
1. **참고문헌 메뉴 → 외부 파일에서 가져오기 (.bib / .ris)…**
2. 파일 선택 다이얼로그 → 파일 픽
...
### 4.5 스마트 머지 (v0.1.10부터)

캐럿이 기존 `[@a]` 그룹에 인접하거나 그 안에 있을 때 새 인용을 삽입하면
**자동으로 합쳐집니다**:

| 기존 | 캐럿 + 삽입 키 | 결과 |
|:--|:--|:--|
| `…[@smith]` (캐럿이 `]` 뒤) | `jones` 삽입 | `…[@smith; @jones]` |
| `[@a; @b]…` (캐럿이 `[` 앞) | `c` 삽입 | `[@a; @b; @c]…` |
| `[@x]` 안에 캐럿 | `y` 삽입 | `[@x; @y]` |
| `[@a; @b]` 안에 캐럿 | `a` 삽입 (중복!) | 거부 + 토스트 `이미 인용되어 있습니다` |

모든 인용 삽입 경로 — `[@`-자동완성, 인용 팔레트, DOI 다이얼로그의
"본문에도 [@key] 삽입", 사이드바 카드 클릭, AI 인용 제안 — 에 동일하게
적용됩니다.
...
v0.1.10부터 참고문헌을 **추가하는 시점에** 기본적으로 abstract MD가
자동 저장됩니다 (`prefs.bibliography.autoSaveAbstract`, 기본 ON).
따라서 카드의 **📥** 버튼은 "더 나은 사본(PDF / 풀텍스트 MD)을
받아오기"라는 의미로 바뀌었습니다.
...
되는 경우는 없습니다. autoSaveAbstract가 켜져 있으면 추가 시점에
이미 stub이 있으므로 📥는 1~4단계의 더 좋은 사본을 시도합니다.
...
우측 사이드바 상단 필터 옆에 **정렬 드롭다운**이 있습니다. 선택값은
`prefs.bibliography.sortBy`에 자동 저장됩니다.

| 옵션 | 의미 |
|:--|:--|
| **추가순 (최신)** *(기본)* | bib 파일에 추가된 순서, 새 것부터 |
| **추가순 (오래된)** | bib에 추가된 순서, 오래된 것부터 |
| **저자 A→Z** | 첫저자 성 알파벳 |
| **연도 (최신)** / **연도 (오래된)** | 출판 연도, 미상은 마지막 |
| **인용키 A→Z** | citation key 알파벳 |
| **본문 인용 순** | 현재 문서에서 첫 등장 순서 (미인용 항목은 마지막) |
| **미사용 우선** | 본문에 한 번도 안 나온 항목부터 (정리·청소용) |
...
**v0.1.10에 추가된 동작 토글** (`preferences.json`의 `bibliography.*`,
Settings UI에서도 차차 노출 예정):

| 필드 | 기본값 | 의미 |
|:--|:--:|:--|
| `insertCitationOnAdd` | `false` | DOI 다이얼로그의 "본문에도 [@key] 삽입" 체크박스의 초기 상태 |
| `autoSaveAbstract` | `true` | 참고문헌 추가 시점에 `reference/<key>.md`로 abstract를 자동 저장 |
| `sortBy` | `addedDesc` | 우측 사이드바 정렬 기본값. 옵션: `addedDesc/Asc`, `author`, `yearDesc/Asc`, `key`, `citationOrder`, `unused` |
...
| 단축키 | 동작 |
|:--|:--|
| `Cmd/Ctrl + Shift + B` | DOI로 참고문헌 추가 (단일) |
| `Cmd/Ctrl + Shift + I` | 본문에 인용 삽입 (fuzzy 검색 팔레트) |
| `[@` 입력 | 본문 내 자동완성 드롭다운 |
| `Cmd/Ctrl + Shift + \\` | 우측 사이드바 (참고문헌/AI) 토글 |
| `F1` | 전체 단축키 목록 다이얼로그 |

참고문헌 메뉴에서 단축키 없이 접근:
- 외부 파일에서 가져오기 (.bib / .ris)
- DOI 일괄 가져오기
- AI가 인용 위치 추천
- 참고문헌 사이드바 열기
