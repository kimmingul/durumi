...
> **v0.1.11 / v0.1.12 추가 — 3 가지 편집 모드**: 두루미는 **WYSIWYG**(기본,
> Word 스타일) / **Typora**(라이브 프리뷰) / **Markdown 소스**의 세 모드를
> 지원합니다. 본 문서의 "라이브 프리뷰" 항목 설명은 *Typora 모드 기준*이며,
> WYSIWYG 모드에서는 활성 라인과 비활성 라인이 동일하게 렌더링됩니다.
> 모드 전환은 상태바의 W/T/M 토글 또는 `Cmd/Ctrl + Shift + 1/2/3` 단축키,
> 보기 → 편집 모드 메뉴 라디오로 가능합니다. 자세한 내용은
> [editor-modes.md](editor-modes.md) 참고.

본 문서를 읽기 전에 알아두면 좋은 약속:

- "라이브 프리뷰"는 두루미 에디터에서 캐럿이 해당 줄을 떠난 직후 보이는
  렌더 결과를 뜻합니다. *Typora 모드 기준*: 캐럿이 해당 줄 안에 있을
  때는 원본 마크다운 소스가 그대로 보입니다(편집 편의성 설계). *WYSIWYG
