...
> **v0.1.11 / v0.1.12 / v0.1.13 — 3 가지 편집 모드**: 두루미는
> **문서 (Document)** *(기본, Word 스타일)* / **라이브 (Live)**
> *(라이브 프리뷰)* / **소스 (Source)** *(평문 마크다운)* 의 세 모드를
> 지원합니다. 본 문서의 "라이브 프리뷰" 항목 설명은 *라이브 모드 기준*이며,
> 문서 모드에서는 활성 라인과 비활성 라인이 동일하게 렌더링됩니다.
> 모드 전환은 상태바의 D/L/S 토글 또는 `Cmd/Ctrl + Shift + 1/2/3` 단축키,
> 보기 → 편집 모드 메뉴 라디오로 가능합니다. (v0.1.13 이전 명칭:
> WYSIWYG / Typora 스타일 / Markdown 소스). 자세한 내용은
> [editor-modes.md](editor-modes.md) 참고.

본 문서를 읽기 전에 알아두면 좋은 약속:

- "라이브 프리뷰"는 두루미 에디터에서 캐럿이 해당 줄을 떠난 직후 보이는
  렌더 결과를 뜻합니다. *라이브 모드 기준*: 캐럿이 해당 줄 안에 있을
  때는 원본 마크다운 소스가 그대로 보입니다(편집 편의성 설계). *문서
  모드*: 활성/비활성 무관 항상 렌더 결과만 보입니다 (Word 스타일).
...
질문, 셀프 노트(`@todo`, `@stats` 등)를 본문 안에 가볍게 붙이고, 사이드바와
오른쪽 채팅 패널에 모아 볼 수 있습니다.

> 메타데이터(작성자, 답글, 해결됨 상태)는 동일 디렉터리의
> `<doc>.md.comments.json` 사이드카에 저장됩니다. 마크다운 본문 자체는
> 그대로 portable — 사이드카가 없는 환경(다른 에디터, v0.1.2 두루미)에서도
> 모든 메모는 그대로 보입니다.
...
- 사이드바 **메모** 탭에 문서 순서대로 모든 메모가 모입니다(태그 chip +
  ...
  점프하고 오른쪽 채팅 패널의 카드에 포커스가 갑니다.
- 상태바에 `N개 메모` 카운터가 표시됩니다(메모 0개일 때는 표시되지 않음).
  CriticMarkup 변경 사항이 있으면 그 옆에 `+N -N ~N ▮N 💬N` 배지도
  나타납니다(§ 3.10).
...
본문에 `%% … %%`로 적힌 메모는 라이브 프리뷰에서 본문 글이 보이지 않고
줄 끝에 작은 색깔 `💬` 아이콘만 남습니다. 메모 본문은 에디터 오른쪽의
**채팅 패널**에 카드로 그려집니다.

- 카드는 해당 메모가 있는 소스 라인과 수직 정렬됩니다(MS Word 댓글 패널과
  동일한 시각 모델).
- 메모가 1개 이상이면 패널이 자동으로 열립니다. 헤더의 `×` 버튼으로 세션
  동안만 닫아 둘 수 있고, `Cmd/Ctrl + Shift + M`으로 다시 토글합니다.
- 카드 컴포넌트:
  - 헤더: 작성자 chip, 상대 시간(예: "3시간 전" / "3h ago"), 우측 상단의
    "해결됨" 체크박스
  - 태그 드롭다운: `@ai` / `@todo` / `@reviewer` / `@stats` / 무태그 /
    커스텀
  - 본문 textarea(자동 높이) — 입력은 300 ms 디바운스로 마크다운 소스에
    동기화되며, 줄바꿈이 들어가면 자동으로 인라인 ↔ 블록 형식으로 전환
  - 답글 스레드와 Reply / Send / Cancel 버튼
  - 삭제 버튼
- 줄 끝의 `💬` 아이콘을 클릭하면 해당 카드에 포커스가 갑니다(역방향: 카드
  클릭 → 에디터의 해당 라인으로 캐럿 이동).
- 패널 너비는 환경설정 `memoPanel.width` 키에 저장되며 기본값은 320 px입니다.

#### 3.9.7 작성자, 시간, 해결됨, 답글 (사이드카, v0.1.4 Track A)

본문 마크다운만으로는 "누가 언제 적었는지"와 "해결되었는지", "답글이
달렸는지"를 표현할 수 없습니다. 두루미는 마크다운을 그대로 portable하게
유지하기 위해, 같은 디렉터리에 동명 사이드카 JSON 파일을 자동 생성합니다.

- 파일명: `<doc>.md.comments.json`
- 1초 디바운스로 자동 저장(atomic tmp + rename). `Cmd/Ctrl + S`를 누르면
  즉시 flush됩니다. "다른 이름으로 저장"을 하면 사이드카도 새 경로로
  재바인딩됩니다.
- 사이드카가 없으면 모든 카드가 기본값(작성자 = 환경설정 이름,
  resolved=false, 답글 없음)으로 그려집니다 — v0.1.3 이전 문서와 호환.
...
```json
{
  ...
  "memos": {
    "<memoId>": {
      "author": "kimmingul",
      "createdAt": "2026-05-09T10:23:11Z",
      "resolved": false,
      "replies": [
        { "id": "uuid…", "author": "reviewer", "text": "…", "createdAt": "…" }
      ]
    }
  }
}
...
- `memoId` = `cyrb53(body + ":" + tag)`의 앞 12자. 본문이 편집되면 ID가
  바뀌므로, sidecar 스토어가 `migrateId(old, new)` 마이그레이션을 자동
  처리합니다(답글·해결됨 상태가 따라 옮겨감).
- 본문이 사라지거나 ID가 더 이상 매칭되지 않는 항목(orphan)은 7일
  유예 후 자동 삭제됩니다(`pruneOrphans`).
- 답글 ID는 가능한 경우 `crypto.randomUUID()`로 생성됩니다.

#### 3.9.8 패널 그룹화 (v0.1.4 Track A)

채팅 패널 헤더의 그룹 드롭다운에서 다음 4가지를 고를 수 있습니다.

| 값 | 설명 |
| :--- | :--- |
| `라인 순` | 기본. 문서 등장 순서. |
| `태그별` | `@ai`, `@todo`, `@reviewer`, `@stats`, 무태그 등으로 묶음. |
| `작성자별` | 사이드카의 `author` 기준. |
| `상태별` | 미해결 / 해결됨 두 그룹. |

또한 "해결됨 숨김" 토글이 있어, 기본적으로 해결된 메모는 패널에서 숨겨
검토 중인 항목만 보이게 합니다(체크 해제 시 모두 보임). 기본값은
`memoPanel.hideResolvedDefault: true`.

작성자 이름은 `설정 → Author / 작성자` 섹션에서 편집할 수 있습니다(기본값
= OS 사용자명).

#### 3.9.9 내보내기 동작 — 안전 기본값
...
> v0.1.3에서 이전 버전 §3.9.8에 적었던 v2 후보(해결됨 상태, 답글 스레드,
> 작성자 메타, CriticMarkup 트랙체인지)는 모두 v0.1.3~v0.1.4에서 출시되어
> § 3.9.6–3.9.9 및 § 3.10에 반영되었습니다.

---

## 3.10 변경 추적 (CriticMarkup)

v0.1.4 Track B에서 [Fletcher CriticMarkup](https://fletcher.github.io/MultiMarkdown-6/syntax/critic.html)
다섯 가지 연산자를 추가했습니다. 메모(`%% %%`)가 "리뷰어가 본문 옆에 붙이는
질문/노트"라면, CriticMarkup은 "리뷰어가 본문 자체를 어떻게 고치고 싶은지"를
표시합니다(MS Word의 "변경 내용 추적"과 같은 의미).
...
| `{++추가++}` | 삽입 | 초록 밑줄 |
| `{--삭제--}` | 삭제 | 빨강 취소선 |
| `{~~기존~>새 글자~~}` | 치환 | 빨강 취소선(기존) + 화살표 + 초록 밑줄(새 글자) |
| `{==강조==}` | 검토용 강조 | 짙은 노랑 배경(영구 강조 `==text==`와 다른 색조) |
| `{>>코멘트<<}` | 마진 코멘트 | 보라색 `💬` pill 위젯 |

가드 규칙:

- 본문이 비거나 공백뿐이면 거부됩니다.
- 줄바꿈을 포함하면 인식되지 않습니다(인라인 전용).
...
- 사이드바의 5번째 탭 **변경 / Changes**가 추가됩니다. 항목은 종류별
  (insertion / deletion / substitution / highlight / comment) 그룹화되어
  보이고, 각 줄을 클릭하면 에디터가 해당 위치로 점프합니다. 탭 상단에는
  `{== ==}`(리뷰 마크)와 `==text==`(영구 강조)의 차이를 짧게 안내하는
  도움말이 들어 있습니다.
- 상태바: CM 변경 수가 0이 아니면 메모 카운터 옆에 `+N -N ~N ▮N 💬N`
  형태의 배지가 표시됩니다(추가 / 삭제 / 치환 / 강조 / 코멘트 개수).

### 3.10.3 내보내기 매트릭스 — accept vs preserve

기본값은 **모든 변경 수락(accept all changes)**입니다. 의학 매뉴스크립트
제출 시 트랙체인지 흔적이 그대로 남는 것은 위험하기 때문에, 메모와 마찬가지로
"기본은 깨끗한 출력"입니다.

`설정 → "변경 사항 포함(Include track-changes annotations)"` 체크박스를
켜면 preserve 모드가 됩니다.

| 형식 | 기본 (accept) | 변경 사항 포함 (preserve) |
| :--- | :--- | :--- |
| HTML / PDF | `{++X++}` → `X`<br>`{--X--}` → 사라짐<br>`{~~A~>B~~}` → `B`<br>`{==X==}` → `X`(언랩)<br>`{>>X<<}` → 사라짐 | `<ins>X</ins>` / `<del>X</del>` / `<del>A</del> → <ins>B</ins>` / `<mark>X</mark>` / `<aside class="cm-comment">X</aside>` |
| DOCX | 동일하게 accept | Pandoc fenced div + 스팬: `[X]{.insertion}`, `[X]{.deletion}`, `[X]{.highlight}`, `::: comment\nX\n:::` |
| LaTeX | 동일하게 accept | DOCX와 동일한 Pandoc 표기. 사용자 지정 LaTeX 클래스에 매핑 가능 |

내부 처리 순서가 중요합니다. **메모 정책(`stripComments` / `promoteComments`)이
먼저 적용된 뒤** CriticMarkup 변환(`transformCm`)이 실행됩니다. 그래야
`%% {++ 변경 ++} %%`처럼 메모가 CM을 감싸고 있을 때 외곽 메모를 먼저 제거할
수 있습니다.

### 3.10.4 `{== ==}` vs `==text==`

두 문법은 시각적으로 비슷하지만 의미가 다릅니다.

| 표기 | 의도 | 기본 export 동작 |
| :--- | :--- | :--- |
| `==text==` (InlineExtras) | **영구 강조** — 최종 문서에 남겨야 할 형광펜 | HTML/PDF에서 `<mark>text</mark>` 그대로, DOCX는 Pandoc highlight |
| `{== text ==}` (CriticMarkup) | **리뷰 마크** — "여길 봐 줘" 같은 임시 표시 | accept 모드(기본)에서 마크가 사라지고 일반 텍스트만 남음 |

라이브 프리뷰도 두 노랑을 다른 색조로 그려서 구별이 됩니다(CriticMarkup
쪽이 더 진합니다). 영구 강조가 필요하면 `==text==`를, 리뷰용 임시 표시는
`{== text ==}`를 사용하세요. 둘 다 보존하고 싶다면 Settings에서 "변경 사항
포함"을 ON으로.

### 3.10.5 취소선과의 충돌 회피

- `~~strike~~`(GFM 취소선)과 `{~~ ~> ~~}`(CriticMarkup 치환)은 서로 독립적
  입니다 — 후자는 반드시 앞이 `{`로 열려 있기 때문에 파서가 명확하게
  구분합니다.
- 그래서 두루미는 CriticMarkup 인라인 파서를 lezer 단계에서 Emphasis,
  Strikethrough, Subscript, Highlight보다 **먼저(`before:`)** 등록합니다.
  그렇지 않으면 GFM Strikethrough가 `~~`를 먼저 먹어 버려 `{~~ ... ~~}`가
  깨집니다.

### 3.10.6 종합 예시

```markdown
이 문장에는 {++삽입++}, {--삭제--}, {~~old~>new~~} 치환,
{==검토 표시==}, {>>옆 노트<<}가 모두 들어 있습니다.
```

라이브 프리뷰:

> 이 문장에는 <ins style="color:green">삽입</ins>,
> <del style="color:red">삭제</del>,
> <del style="color:red">old</del>→<ins style="color:green">new</ins> 치환,
> <mark>검토 표시</mark>, [💬 옆 노트] 가 모두 들어 있습니다.

기본 export(accept all):

```text
이 문장에는 삽입, , new 치환, 검토 표시, 가 모두 들어 있습니다.
```

(삭제와 코멘트는 사라지고, 치환은 새 글자만 남습니다. 강조는 평문으로 떨어집니다.)
...
- **`%% 메모 %%` 매뉴스크립트 메모(§ 3.9)**: MS Word 댓글에 해당하는
  ...
  v0.1.3에서 본문이 줄 끝 `💬` 아이콘으로 축약되고 오른쪽 **채팅 패널**에
  카드로 그려지는 MS Word 스타일 워크플로가 추가되었으며, v0.1.4에서
  작성자 chip · 상대 시간 · 해결됨 체크 · 답글 스레드 · 그룹화(라인/태그/
  작성자/상태)가 사이드카(`<doc>.md.comments.json`) 기반으로 도입되었습니다.
- **CriticMarkup 변경 추적(§ 3.10)**: `{++ ++}` / `{-- --}` / `{~~ ~> ~~}` /
...
- **메모를 한 번 편집했더니 답글이 사라진 것처럼 보임**: 메모 ID는
  `body+tag` 해시이므로 본문이 바뀌면 ID가 바뀝니다. 사이드카 스토어가
  자동으로 `migrateId(old, new)`를 수행하지만, 1초의 디바운스 동안 빠른
  연속 타이핑으로 새 ID가 여러 번 바뀌면 일시적으로 답글이 매칭되지 않은
  것처럼 보일 수 있습니다. 잠시 기다리거나 `Cmd/Ctrl + S`로 즉시 flush
  하세요. 또한 다른 사람이 외부에서 사이드카 파일을 편집했다면 두루미를
  reload해야 반영됩니다.
- **`{== ==}`로 강조했는데 export에서 평문이 됨**: 기본 동작이 "모든 변경
...
CriticMarkup 인라인 파서는 lezer 단계에서 Emphasis / Strikethrough /
Subscript / Highlight보다 **반드시 먼저(`before:`)** 등록되어야 합니다 —
그렇지 않으면 GFM Strikethrough가 `{~~ ... ~~}`의 안쪽 `~~`를 먼저 소비해
파서가 동작하지 않습니다.
