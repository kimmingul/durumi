---
title: 두루미 문서 모드 IME 테스트
author: 김민걸
date: 2026-05-13
description: v0.1.12에서 모든 widget이 활성 줄에서도 렌더되는지 + 한글 IME 합성이 깨지지 않는지 확인하는 종합 테스트. v0.1.13에서 모드 명칭이 변경됨 (WYSIWYG → 문서).
---

# 두루미 문서 모드 종합 테스트

이 문서는 **문서 모드** (v0.1.13에서 "WYSIWYG"에서 명칭 변경)에서 모든 widget이 활성/비활성 라인에 무관하게 동일하게 렌더되는지, 그리고 각 widget 주변에서 **한글 IME composition**이 안전하게 작동하는지를 검증하기 위한 마크다운입니다.

테스트 방법:
1. 상태바 D 아이콘 또는 `Cmd+Shift+1`로 **문서 모드** 진입
